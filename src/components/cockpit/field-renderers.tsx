...
const LABELS: Record<string, string> = {
  ...
  coherenceRisks: "Risques de coherence", devotionVulnerabilities: "Vulnerabilites Engagement",
  ...
  actionsByDevotionLevel: "Actions par Engagement", riskMitigationActions: "Actions de mitigation",
  ...
  devotionFunnel: "Funnel Engagement", overtonMilestones: "Jalons Overton",
  northStarKPI: "North Star KPI", budgetByDevotion: "Budget par Engagement",
  ...
  productExperienceMap: "Experience par produit", ladderProductAlignment: "Engagement x Produit",
  ...
  pilierImpact: "Pilier impacte", devotionImpact: "Impact Engagement", overtonShift: "Shift Overton",
  ...
  duree: "Duree", budget: "Budget", objectifDevotion: "Objectif Engagement",
  ...
  devotionPotential: "Potentiel Engagement", rank: "Rang", prix: "Prix", cout: "Cout",
  ...
  // R — Risk
  blockingPerception: "Perception bloquante", devotionLevelBlocked: "Niveau Engagement bloque",
  ...
  ambassadeurs: "Ambassadeurs", evangelistes: "Prescripteurs",
  acquisition: "Acquisition", conversion: "Conversion",
  retention: "Retention", evangelisation: "Recommandation",
...
export function HeroCard({ label, value, accent }: { label: string; value: string; accent: string }) {
  return (
    <Card span={2} className={`border-accent/20 bg-accent/5`}>
      <Label>{label}</Label>
...
export function QuoteCard({ label, value }: { label: string; value: string }) {
  return (
    <Card>
      <Label>{label}</Label>
      <blockquote className="border-l-2 border-accent/50 pl-3 italic text-white/90 text-sm">
        &laquo; {value} &raquo;
      </blockquote>
    </Card>
  );
...
export function FocusModal({ item, onClose }: { item: Record<string, unknown>; onClose: () => void }) {
  return (
    <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <div className="relative mx-4 max-h-[80vh] w-full max-w-lg overflow-y-auto rounded-xl border border-white/10 bg-background p-6 shadow-2xl" onClick={e => e.stopPropagation()}>
          ...
          <div key={i}
            ...
            <div className="flex items-center justify-between mb-1.5">
              ...
              <div className="flex items-center gap-1.5">
                {p.rank === 1 ? <span className="rounded-full bg-accent/15 px-1.5 py-0.5 text-[9px] text-accent">Primary</span> : null}
              ...
              <div className="rounded-lg border border-white/5 bg-white/[0.02] p-3">
                ...
                <div className="space-y-1">
                  {(myth.sacredRituals as string[]).map((r, i) => (
                    <div key={i} className="flex items-start gap-2 text-xs text-white/70">
                      <Flame className="h-3 w-3 text-accent shrink-0 mt-0.5" />{typeof r === "string" ? r : extractLabel(r as Record<string, unknown>)}
                    </div>
                  ))}
                </div>
              </div>
