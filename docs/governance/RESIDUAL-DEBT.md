...
- **Inbox unifiée S3 — engagement des tiers (mandat « rival de Sprout Social », confirmé 12/07 soir)** : `SocialInboxItem` (commentaires/DM/mentions AVEC identités des auteurs, réponse in-app, assignation ADR-0129, saved replies), service DISTINCT de social-connect (le test 8 verrouille la boucle passive, pas l'Inbox). Lecture des commentaires FB (`pages_read_engagement`) + IG médias propres (`instagram_basic`) possible dès MAINTENANT en polling mode testeurs ; réponses/DM = `pages_manage_engagement` + `instagram_manage_comments` + messaging → à GROUPER avec les scopes publishing (`pages_manage_posts`, `instagram_content_publish`) et insights (`read_insights`, `instagram_manage_insights`) dans UNE seule 2ᵉ soumission App Review ; webhooks temps réel = Advanced Access (trio Meta). AVANT activation : mise à jour /privacy + /data-deletion (traitement des interactions adressées à la marque, rôle processor).
