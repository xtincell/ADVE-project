...
- **`describeScores` / `BrandScoreTriplet` sans appelant de production (LOW)**. Le module `domain/brand-scores.ts` a livré la lecture canon (`readCompositeFromVector`, 5 sites) mais sa pièce maîtresse — la forme nommée à trois mesures — n'est référencée que par son propre test. **Fermeture** : la servir depuis la surface qui expose « le score » (MCP `getBrandCard`, KPI cockpit). **Déclencheur** : prochaine surface qui expose plusieurs mesures à la fois.
