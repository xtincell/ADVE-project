...
| `tests/unit/governance/adr-uniqueness.test.ts` (v6.18.9) | Aucune paire d'ADRs ne partage le même préfixe 4-digit | CI |
| `tests/unit/governance/sequence-lifecycle.test.ts` (v6.18.14) | SequenceLifecycle DRAFT/STABLE/DEPRECATED + SequenceMode + pas de retour à `refined`/`_oracleEnrichmentMode` | CI |
...
| `scripts/audit-llm-chunking-candidates.ts` (Sprint 1.3) | Sites LLM single-call à risque de troncature (≥4000 tokens / ≥15 fields sans chunking) | manuel |
