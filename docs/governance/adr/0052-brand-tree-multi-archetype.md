# ADR-0052 — Brand Tree multi-archétype hiérarchique

**Status** : Accepted
**Date** : 2026-05-06
**Phase** : 18 — Matanga × FrieslandCampina × Brand Tree
**Supersedes** : aucun
**Related** : [ADR-0023](0023-operator-amend-pillar.md) (OPERATOR_AMEND_PILLAR ADVE-only), [ADR-0037](0037-country-scoped-knowledge-base.md) (Country-scoped knowledge base, à étendre node-scoped en Phase 18 noyau), [ADR-0053](0053-llm-as-ui-orchestrator-manual-first.md) (manual-first invariant), [ADR-0054](0054-brand-nature-archetypes-template.md) (cascade par BrandNature), [ADR-0055](0055-morning-brief-batch-validation.md) (Morning Brief Batch dépend de Brand Tree)
...
- La sémantique multi-archétype : FrieslandCampina = `BrandNature: PRODUCT`, mais demain l'OS doit ouvrir des dossiers `CHARACTER_IP`, `MEDIA_IP`, `FESTIVAL_IP`, `PERSONAL`, etc. (cf. [ADR-0054](0054-brand-nature-archetypes-template.md)).
...
model BrandNode {
  ...
  // Classification
  nodeKind    String        // libre, validé contre archétype de nodeNature au runtime (cf. ADR-0054)
...
**Choix de design** :
- `nodeKind: String` libre (pas enum strict) → permet l'évolutivité par archétype sans migration Prisma. Validation contre `BRAND_NATURE_ARCHETYPES` const TS au runtime ([ADR-0054 §1](0054-brand-nature-archetypes-template.md)).
...
### §5 — Manual-first parity (cf. [ADR-0053](0053-llm-as-ui-orchestrator-manual-first.md))
...
- Glory tools LLM (Morning Brief Batch — cf. [ADR-0055](0055-morning-brief-batch-validation.md)) qui orchestrent ces mêmes endpoints
...
4. **Multi-archétype prêt** : `nodeNature` permet d'ouvrir demain des dossiers SERVICE/CHARACTER_IP/FESTIVAL_IP/MEDIA_IP/PERSONAL/INSTITUTION (cf. [ADR-0054](0054-brand-nature-archetypes-template.md)).
5. **Manual-first parity respectée** : aucune feature LLM ne contourne l'UI ; tout est saisissable manuellement (cf. [ADR-0053](0053-llm-as-ui-orchestrator-manual-first.md)).
