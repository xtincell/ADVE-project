...
## Audit UX Cockpit v2 — lots 10/11/12 shippés, restants 13-15 (2026-07-11, NEFER)

Réf : [docs/audits/COCKPIT-UX-AUDIT-2026-07-11.md](../audits/COCKPIT-UX-AUDIT-2026-07-11.md) §F + [ADR-0122](adr/0122-cockpit-founder-nav-ia.md). Shippés : **lot 12** (frontière founder/opérateur — `requireOperator` réparé opt-in, `BatchActionsBar` gardée, 10 segments `<OperatorSurface>`, sources founder-lecture), **lot 10** (nav 35→20 items / 6 groupes, hubs piliers, `activePrefixes`, tabbar `mobileTab` + i18n, persistance marque, Cmd+O, assistant réel), **lot 11** (vocabulaire corps-de-page + test HARD `cockpit-vocabulary.test.ts`).

Restant (non-bloquant, traçable) :

- **`requireOperator` défaut site-par-site** : le flag est réparé en opt-in explicite (défaut `false` honnête). Le durcissement du défaut à `true` exige un audit fonctionnel de chaque `governedProcedure` (guilde/paiements/candidatures doivent rester non-opérateur) — chantier dédié, ne pas flipper le défaut à l'aveugle.
- **Lot 13 — consolidation surfaces redondantes** : Oracle 1 seule surface d'assemblage (retirer le bloc legacy `enrichOracle` de l'UI après audit ADR-0073, cohabitation aujourd'hui gardée opérateur-only) ; fusion réelle Calendrier/operate-roadmap et onglets Livrables (Guidelines/Assets) + Rapports (Diagnostics/Benchmarks/Attribution) — aujourd'hui consolidés en nav par `activePrefixes` seulement ; settings 1 seule section Abonnement ; dé-densification campagne `[id]` (~2 270 l. / 12 onglets).
- **Lot 14 — lexique & feedback** : trancher le lexique T7 (Prescripteur/Évangéliste, engagement/dévotion, Indice d'attachement, Résilié/Annulé, FCFA partout) + module `TIER_LABELS` unique settings/billing (clés ET libellés divergents) + 10 `alert()/confirm()` natifs → dialogs DS (UX-DR14/15) + labels du helper `getPillarChipStatus` (« FULL » EN).
- **Lot 15 — DS & i18n résiduels** : 16 occurrences couleurs brutes cockpit (top : `operations-center.tsx`, `pillar-page.tsx` hex inline), clés i18n hors nav, accents résiduels.
- **Lot 0 (inchangé, DÉFÉRÉ OPÉRATEUR)** : statut client-facing des lettres ADVE/RTIS/A…S — le test vocabulaire les exclut volontairement ; les hubs affichent la lettre en badge réversible.
- **Purge vocab surfaces opérateur-gated** : `apogee-maintenance` (« Loi 4 APOGEE ») reste en allowlist justifiée du test vocabulaire — purger ou supprimer la route au lot 13/15.
