...
- **Phase 23 — Câblage des mécaniques pivot mission (superfans × Overton) MVP→PRODUCTION** — 📋 **PLANNED, READY FOR IMPLEMENTATION**. Closure-roadmap target #1 (anciennement Phase 22 pré-rename 2026-05-15). PRD + UX + architecture livrés (`_bmad-output/planning-artifacts/`). 9 décisions D1-D9 + 7 patterns P22-1..7 + 5 child ADRs réservés 0077-0081. Surface réelle (après correction PRD scope par architecture D1) : 2 connector façades Tarsis API + CRM via Credentials Vault + 1 lifecycle Intent paramétré `PROMOTE_PIVOT_SUBCLUSTER` + ML calibration pure-TS sans dépendance + manual-first parity HARD-test + 1 Cockpit route mountant `<OvertonRadar>` existant. 0 new Prisma model. Cap APOGEE 7/7 préservé. Étape suivante : `/bmad-create-epics-and-stories`.
