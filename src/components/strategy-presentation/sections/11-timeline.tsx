  ...
  return (
    <div className="space-y-6">
      {data.campaigns.length > 0 && data.campaigns.some((c) => c.milestones.length > 0) && (
        <div>
          <h3 className="mb-3 text-sm font-semibold uppercase tracking-wider text-foreground-secondary">Retroplanning</h3>
          {data.campaigns.filter((c) => c.milestones.length > 0).map((c, i) => (
            <div key={i} className="mb-4">
              <h4 className="mb-2 text-sm font-semibold text-foreground-secondary">{c.name}</h4>
              <DataTable
      ...
      {data.teamMembers.length > 0 && (
        <div>
          <h3 className="mb-3 text-sm font-semibold uppercase tracking-wider text-foreground-secondary">Gouvernance</h3>
          <DataTable
