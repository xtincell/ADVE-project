...
- **`QuickIntake.advertis_vector.composite` est sur une échelle /100 (MED, piège)**. L'intake persiste un composite 4 piliers × 25 sous la **même clé** que le canon /200. `readCompositeFromVector` le servirait avec `max: 200` le jour où une surface pointe sur une ligne d'intake. Aucune ne le fait aujourd'hui. **Fermeture** : renommer la clé côté intake, ou porter l'échelle dans le vecteur.
...
- **NOMBRES top-level `ai_generation` (MED, pré-existant, anti-fab)** *(finding adversarial v6.27.325)* : `deriveSchemaRequirements` marque tout champ NOMBRE top-level `derivable:true, derivationSource:"ai_generation"` (`pillar-maturity-contracts.ts:640-641`) → `A.turnoverRate` et `I.totalActions` (les 2 `is_number` COMPLETE derivables) atteignent le LLM via la liste `derivable` standard (avant ET après ce chantier — hors périmètre `optionalFillable`, qui est prouvé number-free). Un nombre fabriqué = donnée réelle inventée (interdit n°3). Pire pour `I.totalActions` : `deriveByCalculation` a un compteur déterministe correct (`auto-filler.ts:598-608`) rendu MORT par le routage `ai_generation`. **Fermeture** : marquer `turnoverRate` `needsHuman` (donnée réelle de churn) + router `totalActions` en `derivationSource:"calculation"` (son compteur existe). Score-safe (le req reste compté ; seul le split derivable/needsHuman/calculation change). **Déclencheur** : prochaine passe anti-fabrication sur les nombres du contrat, ou si l'opérateur voit un `turnoverRate`/`totalActions` inventé. Effort ~¼ session. *(NB : la couche `optionalFillable` de ce chantier applique déjà `isNonFabricableLeaf` — nombres/booléens/computed/refs jamais fabriqués ; ce résidu concerne le chemin NOMBRE top-level du contrat, distinct.)*
