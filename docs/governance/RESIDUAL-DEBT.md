...
- **IDOR entité-id LOW non fixés** *(sweep round-4/5)* : reads `operatorId`-keyés cross-opérateur restants (`campaign-deliverable.listForOperator/statsForOperator`, `campaign-change-request.listOpenForOperator`, `phase18-residuals.*`) ; `anubis.trackDelivery/fetchDeliveryReport` (`broadcastJobId`, métriques de diffusion cross-opérateur) ; `media-buying.syncToCampaign` (`campaignId` leurre — écrit une CampaignAmplification sur une campagne non vérifiée) ; `knowledge-graph.query` (énumération text-search de toutes les stratégies). **Hors-classe (accès cassé cross-user/contenu global, pas tenant-IDOR strict)** : `boutique.updateOrderStatus` (marque une commande payée/expédiée), `editorial.publish/create`, `event.markAttended/create`, `learning.issueCertification/publishCourse` (tout authentifié s'auto-délivre un certificat / publie du contenu global). **Plan** : même patron (résoudre id→propriétaire→`canAccess*` / gate operator-admin) ; les hors-classe demandent une politique produit (qui publie l'éditorial/événements/cours). **Déclencheur** : prochaine passe sécurité (LOW) OU décision produit (hors-classe).
