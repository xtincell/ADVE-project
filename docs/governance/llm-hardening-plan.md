...
| **1** | PROD | **Verrou de sortie** (+ entrée sur les sites qui contournent les chokepoints LOT 0). Sous-lots : **1a** points d'intake ✅ (`deduce-adve`, `boot-sequence`, `narrate-adve`, `rtis-draft`, `brief-ingest`) ; **1b** rtis-cascade/notoria/mestor-insights ✅ (entrée) ; **1c** Glory ✅ **37/37 faits** (schémas de sortie *réels* dérivés des promptTemplate, `glory-output-schemas.ts` + phase19-tools.ts ; baseline gate ratchetée 37→0 ; **76/76 nœuds LLM/HYBRID + 28/28 frameworks à 100 %**) ; **1e** campaign-plan/vault-enrichment/reliquat **entrée** (52 appels / 38 fichiers, gelés au baseline). *(Note : tous les Glory tools ont déjà l'entrée durcie via LOT 0/`engine.ts` ; il ne reste que l'entrée des services directs hors chokepoint = 1e.)* | 🟢 1a+1b+1c faits |
| **2** | PROD | **Gate CI** : `audit:llm:strict` branché dans `ci.yml` (job *LLM node guardrails*, régression-only vs baseline). | ✅ |

> **État sécurité (2026-06-23)** : l'**injection de prompt (entrée) est fermée sur tout le périmètre** — outils Glory (LOT 0/engine.ts), frameworks (LOT 0/artemis), points d'intake (LOT 1a), services de dérivation (LOT 1b). Le **verrou de sortie est complet sur les Glory tools** : **37/37 outils + 76/76 nœuds LLM/HYBRID + 28/28 frameworks à 100 %** (LOT 1c clos, baseline sortie = 0). Le **gate CI (LOT 2)** empêche toute régression. **Reste = entrée des services directs hors chokepoint** (LOT 1e : 52 appels / 38 fichiers) — *gelés au baseline* et à résorber au fil de l'eau.
