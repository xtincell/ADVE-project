...
- **B1 — Q3 non-bypass réellement enforcé** : le lint `lafusee/no-direct-service-from-router` est `warn` + un marqueur de fichier (`lafusee:governed-active`) exempte 101/104 routeurs sans VÉRIFIER qu'ils gouvernent vraiment ; le test `yggdrasil-three-invariants` grep sa propre règle (tautologie). **Fermeture** : test HARD qui, pour chaque routeur `governed-active`, assère que chaque `.mutation()` est `governedProcedure`/`auditedProcedure`/`emitIntent`-in-body ; puis flip lint + `audit-governance router-bypass` à `error`. **Risque** : flip → peut révéler de vraies violations bloquant le build → à faire dans une passe dédiée (auditer les 101). **Déclencheur** : chantier « durcissement Q3 ».
- **B2 — mutations directes non gouvernées** : `actions.setSelected`/`setTiming`/`autoSchedule` (décision opérateur feeding pilier-S), `brand-mcp.revokeKey`/`createKey`, `mcp-billing.setKeyActive`/`updateKeyBilling`/`settleStatement` font des `db.*` directs dans `.mutation()` sans émission (Q1/Q2 absents). **Fermeture** : router via `governedProcedure` (kind `SET_BRAND_ACTION_STATUS` existe déjà pour actions) ou envelopper le routeur en `auditedProcedure`. **Déclencheur** : même chantier Q3 (B1).
- **B3 — 13 kinds émis hors `INTENT_KINDS`/SLOs** (`ENRICH_R_FROM_ADVE`, `ENRICH_T_FROM_ADVE_R_SESHAT`, `GENERATE_I_ACTIONS`, `SYNTHESIZE_S`, `PROPOSE_ADVE_UPDATE_FROM_RT`, `PROPOSE_BRAND_ACTIONS`, `CAPTURE_INTENTION`, `GENERATE_BRIEF_FROM_INTENTION`, `VALIDATE_INTENTION_BRIEF`, `PRODUCE_DELIVERABLE`, `INDEX_BRAND_CONTEXT`, `RUN_MARKET_RESEARCH`, `PROCESS_SESHAT_SIGNAL`) : dispatchés (Q1 trace OK) mais échappent au monitoring SLO + cost-gate (Loi 3). **Fermeture** : les enregistrer dans `INTENT_KINDS` + `slos.ts` (SANS capability cost-gate → pas de nouveau veto) + test HARD union/dispatch ⊆ `INTENT_KINDS` (scan source du switch commandant). **Déclencheur** : passe registre Intent.
...
- **C3 — provenance ingestion brand book** : le persister écrit tous les champs en `SOURCE` (fait observé), y compris ceux extraits par LLM — indistinguables du parseur déterministe. Mitigé par la revue opérateur (preview→confirm avant écriture). **Fermeture** : porter le mode d'extraction (LLM vs STRUCTURED) sur l'Intent `INGEST_BRAND_BOOK` → provenance `INFERRED` pour les champs LLM non re-vérifiés (`SOURCE` pour le déterministe). **Déclencheur** : prochaine passe ingestion (change le payload d'Intent).
- **G — moteur de restauration réel (Compensate)** : les 5 kinds compensateurs non-palier (`ROLLBACK_PILLAR`/`ROLLBACK_ADVE`/`ROLLBACK_RTIS_CASCADE`/`DISCARD_RECOMMENDATIONS`/`REVERT_RECOMMENDATIONS`) n'ont pas de handler → le bouton Compensate console n'enregistre qu'une ligne d'audit (`executed:false`). **Le mensonge UI est FERMÉ** (l'opérateur est prévenu « audit uniquement — rien restauré »). Reste à BÂTIR la restauration réelle depuis l'historique `PillarVersion` (Loi 1 : les snapshots existent). **Fermeture** : handler `ROLLBACK_PILLAR` (restaure `Pillar.content` depuis la `PillarVersion` antérieure via le gateway) + registration 5-sites + wiring compensate → emitIntent ; ADVE/RTIS_CASCADE = multi-piliers ; DISCARD/REVERT = état reco. **Déclencheur** : chantier dédié « moteur de compensation » (feature, pas un trou — admin-gated, faible blast radius).
