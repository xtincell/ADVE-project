...
- `ENRICH_R_FROM_ADVE` → mestor (sync) — Dérive le pilier R (Résonance) depuis l'ADVE — étape cascade RTIS.…
- `ENRICH_T_FROM_ADVE_R_SESHAT` → mestor (sync) — Dérive le pilier T (Territoire) depuis ADVE + R + digest Seshat — étape cascade …
- `GENERATE_I_ACTIONS` → mestor (sync) — Dérive le pilier I (Innovation/actions) depuis la cascade — étape RTIS.…
- `SYNTHESIZE_S` → mestor (sync) — Synthétise le pilier S (Stratégie) — étape finale de la cascade RTIS.…
- `PROPOSE_ADVE_UPDATE_FROM_RT` → mestor (sync) — Propose une mise à jour ADVE à partir des dérivés R/T (décision opérateur — STOP…
...
- `PROPOSE_BRAND_ACTIONS` → artemis (sync) — Propose des BrandAction additives (Phase 24) — LLM brand-aware ou saisie manuell…
- `PRODUCE_DELIVERABLE` → artemis (sync) — Exécute une GlorySequence pour produire un livrable (teaser post-intake / comple…
- `CAPTURE_INTENTION` → intention (sync) — Capture l'intention founder (aval de l'ADVE) — Phase 24 (ADR-0106).…
- `GENERATE_BRIEF_FROM_INTENTION` → intention (sync) — Génère un brief à partir de l'intention capturée — Phase 24.…
- `VALIDATE_INTENTION_BRIEF` → intention (sync) — Valide le brief d'intention avant production — Phase 24.…
...
- `INDEX_BRAND_CONTEXT` → seshat (async) — Indexe le contexte de marque dans le context-store (RAG) — indexeur async Seshat…
- `RUN_MARKET_RESEARCH` → sector-intelligence (sync) — Recherche marché cross-brand (fetch → extract → persist) — Phase 20.…
- `PROCESS_SESHAT_SIGNAL` → sector-intelligence (sync) — Traite un signal Seshat (télémétrie sectorielle).…
...
- `ADMIN_TRANSFER_BRAND_OWNERSHIP` → accounts (sync) — Console superviseur : transfere la PROPRIETE d'une marque a un compte EXISTANT (…
- `ADMIN_RESET_USER_PASSWORD` → accounts (sync) — Console superviseur : pose un mot de passe PROVISOIRE sur un compte existant (bc…
- `ADMIN_PURGE_USER_ACCOUNT` → accounts (sync) — Console superviseur : supprime un compte cree par erreur (faute de frappe sur l'…
...
- `LEGACY_CAMPAIGN_MANAGER_CREATE` → campaign-manager (sync) — Strangler-promoted mutation 'create' from router 'campaign-manager'.…
...
- `LEGACY_CAMPAIGN_MANAGER_CREATE_FROM_VALIDATED_ACTION` → campaign-manager (sync) — Strangler-promoted mutation 'createFromValidatedAction' from router 'campaign-ma…
...
- `LEGACY_CAMPAIGN_MANAGER_ROUTE_TO_GUILDE` → campaign-manager (sync) — Strangler-promoted mutation 'routeToGuilde' from router 'campaign-manager'.…
...
- `LEGACY_NOTORIA_GENERATE_FROM_VAULT` → notoria (sync) — Strangler-promoted mutation 'generateFromVault' from router 'notoria'.…
- `LEGACY_NOTORIA_GENERATE_TYPED_RECOMMENDATIONS` → notoria (sync) — Strangler-promoted mutation 'generateTypedRecommendations' from router 'notoria'…
