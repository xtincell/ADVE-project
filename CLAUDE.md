...
- **Phase 18 (suite — Brand Tree multi-archétype + Matanga × FrieslandCampina)** ([ADR-0052](docs/governance/adr/0052-brand-tree-multi-archetype.md) + [ADR-0053](docs/governance/adr/0053-llm-as-ui-orchestrator-manual-first.md) + [ADR-0054](docs/governance/adr/0054-brand-nature-archetypes-template.md) + [ADR-0055](docs/governance/adr/0055-morning-brief-batch-validation.md)) — 🔵 J1 shipped (2026-05-06), sprint 18-A0 en cours. Plan complet dans [docs/governance/plans/PHASE-18-MATANGA-FC.md](docs/governance/plans/PHASE-18-MATANGA-FC.md). Refonte structurante : `Strategy` plat → arbre de marque hiérarchique multi-archétype (9 BrandNature : PRODUCT/SERVICE/CHARACTER_IP/FESTIVAL_IP/MEDIA_IP/RETAIL_SPACE/PLATFORM/INSTITUTION/PERSONAL). Cascade FMCG 7 niveaux (CORPORATE → MASTER_BRAND → REGIONAL_CLUSTER → REGIONAL_BRAND → PRODUCT_LINE → PRODUCT_VARIANT → SKU). Driver business : ingestion FrieslandCampina (1 corporate + 6 master brands + 4 clusters géo + 15 pays + ~30 SKU) + dashboard agence cross-clients Afrique pour Matanga. **Invariant transverse Manual-first parity (ADR-0053)** : toute feature LLM doit avoir UI manuelle équivalente, LLM orchestre via mêmes endpoints qu'opérateur humain. Phasage : 18-A0 (8-10j) Brand Tree min + 3 vues dashboard ; 18-A1 (5-7j) Morning Brief Batch ; 18-A2 (4-5j optionnel) auto-pull connectors ; 18 noyau (14-18j) héritage piliers + RAG arborescent + variable bible reclassif ; 18-bis (3 mois) M&A + archétypes non-PRODUCT. **Cap APOGEE 7/7 préservé** — aucun nouveau Neter, sous-domaine de Mestor governance. Doctrine LLM NEFER §1.1 enrichie en parallèle (pas de notion de temps humain, pas d'économie de tokens, pas de fatigue).
