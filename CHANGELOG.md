...
## v6.27.128 — feat(forge): COMPOSE_DELIVERABLE passe de l'aperçu au forge réel ([ADR-0136](docs/governance/adr/0136-compose-deliverable-dispatch.md)) (2026-07-13)
...
- **(b) Dé-densification `operate/campaigns/[id]`** : 2 327 l. / 12 onglets monolithiques → parent 197 l. + `campaign-tabs/` (12 onglets + shared, extraction verbatim — clés de cache tRPC et cascades d'invalidation à l'identique), tab bar regroupée en 5 jobs founder (Pilotage / Contenu & production / Diffusion / Mesure / Réglages), keys/URLs inchangées.
...
## v6.27.89 — fix(cockpit): lots 13-15 — consolidation, lexique & feedback, DS résiduel (2026-07-11)

**Lot 13 (consolidation)** : page Oracle = une seule surface founder (console live, tracker NSP et partage personas passent opérateur-only ; le founder garde stats, « Ouvrir le livrable », panel progressif, exports) ; settings = une seule section Abonnement (essentiel + lien billing, le détail vit sur billing) ; **Calendrier unique** (`/operate/calendar` héberge Lancement + Plan d'actions en onglets, `/operate/roadmap` → 308 `?vue=actions`). **Lot 14 (lexique & feedback)** : module unique `src/lib/billing/subscription-labels.ts` (settings/billing divergeaient jusque dans les clés — GROWTH/SCALE fantômes ; canon = clés de `pricing-tiers.ts`, « Résilié ») ; lexique tranché : Prescripteur (dashboard + échelle partagée), « échelle d'engagement », labels FR de l'échelle accentués, chip « FULL » → « OPTIMAL » ; **les 10 `alert()/confirm()` natifs remplacés** (ConfirmDialog UX-DR14 pour newsletter/archivage/création/régénération, toasts pour les échecs d'export et fichiers trop lourds). **Lot 15 (DS)** : 14 occurrences couleurs brutes → tokens (`operations-center`, `pillar-page` hex inline → `var(--color-*)`, tracker, missions, newsletter) — 2 exceptions justifiées (styles d'artefacts exportés) ; accents résiduels (topbar Paramètres/Déconnexion, sélecteurs, éditeur). 2436/2436 tests verts.

## v6.27.88 — fix(cockpit): lot 11 — vocabulaire client corps-de-page + verrou CI (2026-07-11)

Suite de l'audit UX v2 ([docs/audits/COCKPIT-UX-AUDIT-2026-07-11.md](docs/audits/COCKPIT-UX-AUDIT-2026-07-11.md) §T1). Les lots 1+9 de juin avaient renommé la sidebar mais pas les corps de pages — le client voyait deux noms pour chaque chose. Purge des chaînes rendues : Gazette (« Jehuty · Sous tutelle Seshat », « Telemetry »), Recommandations (nodes « Jehuty ▸ Notoria », « pipeline ADVERTIS », « function-calling », tooltip « ADR-0090 », enums bruts traduits), Oracle (« Lancer Artemis » → « Assembler la proposition », « par Mestor » → « par votre équipe », « IntentEmission » → « Réf. d'exécution », chemin `/console/...` retiré du log client, badge « ADR-0073 » retiré, bouton legacy rendu opérateur-only), radar sectoriel (« Tarsis » ×3), livrables (« sequences GLORY », « BRANDBOOK-D »), ignition (« fiche ADVE-RTIS » + accents), dashboard (« Northstar » → « Cap superfans », « Score ADVE-RTIS » → « Score de marque », modes de vue FR). **Nouveau test HARD `tests/unit/governance/cockpit-vocabulary.test.ts`** : scan des littéraux + texte JSX du portail cockpit contre les motifs interdits (mythologie, ADVERTIS/APOGEE, IntentEmission, réfs ADR, function-calling), allowlist justifiée pour surfaces opérateur-gated ; les lettres ADVE/RTIS restent hors test (décision Lot 0 déférée). 890/890 tests gouvernance verts.

## v6.27.87 — feat(cockpit): lot 10 — refonte IA de navigation founder, ADR-0122 (2026-07-11)

Le signal opérateur « la navigation est confusante » traité à la racine (audit §B) : **35 items / 11 groupes → 20 items / 6 groupes** orientés jobs founder (Ma marque / Mes livrables / Mon activité / Mon marché / Mon compte). `NavItem.activePrefixes` + résolution « préfixe le plus long gagne » (`nav-active.ts`, partagée sidebar/tabbar) ; 2 hubs piliers `/cockpit/brand/fondation` (A·D·V·E) et `/cockpit/brand/strategie` (R·T·I·S) — les 8 URLs piliers ne bougent pas, l'alias `brand/strategy` 308 → hub ; tabbar mobile en slots explicites `mobileTab` + i18n (fini `slice(0,4)` et les littéraux bruts) ; nav i18n complète FR/EN/ZH ; marque active persistée (localStorage) ; collision `Cmd+K` résolue (sélecteur de marque → `Cmd+O`) ; bouton topbar Assistant = lien réel vers `/cockpit/mestor` (l'état `mestorOpen` n'était consommé par aucun panneau) ; footer sidebar → racine du portail ; copie du tour réalignée. Cf. [ADR-0122](docs/governance/adr/0122-cockpit-founder-nav-ia.md).

## v6.27.86 — fix(cockpit): lot 12 — frontière founder/opérateur (2026-07-11)

Audit UX v2 §T8/[M02-01]. Le flag `requireOperator` de `governedProcedure` était **inerte** (deux branches identiques du ternaire, JSDoc promettant un défaut `true` jamais appliqué) → réparé en opt-in explicite vers `operatorProcedure`, posé sur `pillar.cascadeRTIS` + `pillar.enrichAllFromVault` ; durcissement site-par-site du défaut tracé en dette. La `BatchActionsBar` du dashboard (3 mutations opérateur sans garde — clic founder = FORBIDDEN) n'est plus rendue au founder (UX-DR16). 10 segments passent sous garde `<OperatorSurface>` (operate/center·forge·sequences·action-brief·missions, portfolio, brand/edit, brand/rtis, insights/apogee-maintenance, intelligence/track) : écran honnête « pris en charge par votre équipe », jamais de clic→FORBIDDEN. Sources : founder en lecture (certitude `readOnly`), triage vault/édition/purge/suppression réservés opérateur.

---

## v6.27.85 — fix(governance): spine d'émission unifié — emitIntent hash-chaîné, fail-closed, cost-gated ([ADR-0124](docs/governance/adr/0124-emission-spine-unified.md)) (2026-07-11)
