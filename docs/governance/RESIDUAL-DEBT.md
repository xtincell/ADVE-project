...
### V1 — Faits épinglés — ✅ CLOS (v6.27.383, PR #692)
...
### V2 — Hygiène des populations — ✅ CLOS (v6.27.384, PR #693) · reste : marquer les 41 en quarantaine (opération opérateur, passerelle requise)
...
### V3 — Le tournoi — ✅ CLOS (ADR-0191, v6.27.385, PR #694)
...
### V4 — Restitution & rang — ✅ CLOS côté code (ADR-0192, v6.27.386, PR #695) · rescans du corpus en attente de la passerelle
...
### V5 — Un seul palier — ✅ CLOS (v6.27.387)
...
### V6 — Finitions produit — ✅ canons CLOS (v6.27.388) · R6/R7 restent ouverts
- ~~**Canons Spawt/Motion19 : vocabulaire touchpoints faux**~~ — **CLOS (v6.27.388, V6)**. Remapping DÉTERMINISTE, sans une invention : le rôle réel qui vivait dans `type` (« La vitrine du faire », « Conseil personnalisé + commande + SAV ») passe dans `role` ; `type` reçoit sa nature d'enum (PHYSIQUE/DIGITAL/HUMAIN) déduite du canal ; `stadeAarrr: "Acquisition"` devient `aarrStage: "ACQUISITION"` ; `channelRef` posé quand la taxonomie couvre la plateforme. 14 touchpoints (8 Motion19, 6 Spawt). `devotionLevel` reste ABSENT — il relève d'une déclaration stratégique humaine, jamais d'une déduction.
...
### V7 — Solde du registre historique — 🟡 3 lignes closes (nombres, composite, palier) · lourdes restantes tracées
...
- **L6 — édition par champ : NON FAIT (MED, UX)**. Le formulaire reste monolithique ; le plan prévoyait l'édition en place pilotée par la variable-bible (`editableMode` + `fieldCertainty`), collections comprises (ajout/suppression d'item). La voie d'écriture ne changerait pas (`OPERATOR_AMEND_PILLAR`). **Déclencheur** : demande opérateur d'éditer un item de collection sans passer par le formulaire complet. Effort ~1 session.
- **L8 — mise à jour des landings : NON FAIT**. Demande opérateur (« beaucoup trop de choses ont changé »). Non entamé : c'est un travail de contenu marketing qui demande un arbitrage de positionnement, pas une correction. **Déclencheur** : arbitrage opérateur sur le message à porter. Effort ~1 session.
- ~~**Propriété des marques — exécution PROD à faire**~~ — **CLOS (2026-07-27)**. Les deux voies de provisionnement ne savaient produire que des **collaborateurs délégués** (scopés par le firewall de zones ADR-0131) : les dirigeants à qui l'on remettait leur marque sont restés les délégués de leur propre bien. Trois mécanismes ont refermé l'écart (`attachAs: "OWNER"`, `accounts.transferBrandOwnership`, `/api/admin/prod-finish` qui n'abandonne plus le transfert quand un mot de passe existe déjà), et les deux transferts sont **passés en prod** : `jlombat@gmail.com` → FanTribe, `lionel@motion19.com` → Motion19. **Erreur commise au passage, corrigée** : le premier appel portait `lionel@motion19.cm` (`.cm` au lieu de `.com`) et a donc créé un compte fantôme, propriétaire de Motion19 pendant quelques minutes — d'où `ADMIN_PURGE_USER_ACCOUNT` (fail-closed sur la propriété) et le défaut par défaut de `prod-finish` corrigé. Leçon : un email de destinataire n'est pas inférable, il se fait confirmer.
- **Comptes — mots de passe irrécupérables par construction (INFO, pas une dette)**. `hashedPassword` est une empreinte bcrypt coût 12 : ni l'opérateur ni personne ne peut relire un mot de passe existant. La seule voie honnête est d'en poser un nouveau — `accounts.resetPassword` (provisoire, `passwordChangeInvited`, rendu UNE fois, payload redacté au spine) et `GET /api/admin/accounts` pour l'inventaire (qui possède quoi, qui n'est que délégué, qui peut seulement se connecter). Inscrit ici parce que la question revient et que la réponse « je te les redonne » est fausse.
- ~~**Cohérence de palier (ADR-0167) — partielle (MED)**~~ — **CLOS (v6.27.387, V5)**. Les trois surfaces nommées — widget PUBLIC `/api/widget/score`, portefeuille console, sélecteur cockpit — servent désormais `effectiveTier` (palier officiel s'il est posé, dérivé sinon). Le pire cas était le widget : la régression silencieuse s'affichait chez le client, sur son propre site. `strategy.list` renvoyait déjà `apogeeTier` (`include`, pas `select`) — il suffisait de le lire. Verrou `single-tier-truth.test.ts` : toute surface d'affichage de palier consomme `effectiveTier` et n'appelle plus `classifyTier` nu. **Reste hors périmètre d'affichage** : les usages internes de `classifyTier` (calcul, landings marketing, canon) restent légitimes — c'est la fonction de dérivation, pas un affichage de palier officiel.
- ~~**`describeScores` sans appelant de production**~~ — **CLOS (v6.27.351)** : servi par `advertis.getBrandCard` (champ `scores`), qui lit le dernier `CultIndexSnapshot` et le dernier `ScoreVerdict` de la marque. Un agent voit désormais les trois mesures nommées, avec `null` sur celles qui ne sont pas mesurées. Garde ajoutée au test `score-single-truth` : un canon que personne n'appelle n'est pas un canon. **Reste** : le KPI cockpit sert toujours la seule complétude structurelle (suffisant pour son usage — il affiche « le score de marque »).
- ~~**`QuickIntake.advertis_vector.composite` sur échelle /100 (MED, piège)**~~ — **CLOS (v6.27.388, V7)**. `compositeScaleOf(vector)` rend l'échelle par un discriminant STRUCTUREL, pas heuristique : un vecteur d'intake ne porte que les 4 clés ADVE non nulles (r/t/i/s à 0, les RTIS sont dérivés plus tard), un vecteur canon en a au moins une. Valeur illisible ⇒ /200 (l'échelle canon, jamais une supposition basse). Le piège devient INOFFENSIF avant qu'une surface ne le déclenche.

## Chantier « assessment profond honnête » 2026-07-24 ([ADR-0178](adr/0178-deep-honest-maturity-assessment-decoupled-from-frozen-score.md)) — déférés

- ~~**NOMBRES top-level `ai_generation` (MED, anti-fab)**~~ — **CLOS (v6.27.388, V7)**. `a.turnoverRate` → `needsHuman` (un taux de rotation clients est une donnée d'entreprise réelle, aucune IA ne la connaît) ; `i.totalActions` → `derivationSource: "calculation"` — son compteur EXACT existait dans `auto-filler.deriveByCalculation` (somme des actions du catalogue par canal) et le routage par défaut le rendait MORT : le LLM ESTIMAIT ce que le code savait COMPTER. Posé au vrai générateur (`deriveSchemaRequirements`, pas le glory-derived — première tentative au mauvais endroit, attrapée par le test). Verrou `numbers-are-never-fabricated.test.ts`.
- **Audit renderer per-pilier « champ rempli mais non affiché » (LOW/MED, v6.27.325)** : la couche `optionalFillable` fait qu'Enrichir remplit désormais les cellules OPTIONNELLES de tableau (`personas[i].*`, `produitsCatalogue[i].*`, etc.) IDENTIQUEMENT sur les 8 piliers. Mais chaque pilier a un renderer bespoke (`pillar-*-fields.tsx`) qui liste ses champs explicitement → un champ nouvellement remplissable peut être invisible dans la carte (cas corrigé : D `personas[i].lifestyle`/`familySituation`). **Honnête aujourd'hui** : la donnée est bien écrite (visible dans « Modifier »/Oracle) ; D corrigé, V affiche déjà ses cellules (« — »). **Reste** : auditer A/E/R/T/I/S renderer par renderer — pour chaque sous-clé qualitative de tableau que `findEmptyArrayCellPaths` rend remplissable, vérifier qu'une surface l'affiche, sinon ajouter la ROW (garde `isEmpty`, DS). **Risque à l'ajout** : rewrites de 6 renderers → à faire prudemment, un pilier à la fois. **Déclencheur** : si l'opérateur constate un champ rempli par Enrichir mais absent de la carte d'un pilier ≠ D. Effort ~1 session.
- **Gap B — panneau recos page pilier : « accepter » ≠ « appliquer » (LOW/MED, pré-existant)** : depuis la page pilier, le panneau recos ne fait qu'`acceptRecos` (file) ; l'apply gouverné (`APPLY_RECOS_RESOLVED`) vit au hub Notoria. La notoria PROPOSE déjà les cellules optionnelles (`engine.ts`), mais on ne peut les APPLIQUER depuis la page. Enrichir (auto-filler) est désormais le chemin praticable depuis la page — Gap B reste séparé. **Fermeture** : bouton « Appliquer » per-pilier (`applyRecos` + gate) OU clarifier « Accepter → appliquer au hub ». **Déclencheur** : passe UX du panneau recos. Effort ~½ session.
- **Garde Zod-`safeParse` canon ↔ schema (LOW, prévention DATA complète)** : le test `pillar-completeness-reconciliation` verrouille désormais la classe « valeur cachée sous une variante de CASSE/NOM » (bug `onNeDitPas`) + « feuille requise vide » sur les 4 marques canon. Restent NON couverts par ce garde : les mismatches de **type** (une string là où le schema veut un nombre) ou d'**enum** (valeur hors des `z.enum`) dans un canon — Zod les stripperait/rejetterait au parse, mais aucun test ne fait ce parse aujourd'hui. **Fermeture** : ajouter un test qui `PILLAR_SCHEMAS[k].safeParse(canonContent)` pour chaque marque × pilier (mode non-strict pour tolérer les clés d'affichage extra) et échoue sur toute violation required/type/enum. **Risque à l'ajout** : peut surfacer des canons non-strictement-Zod-valides (à corriger un par un). **Déclencheur** : prochaine touche aux fichiers canon, ou passe « intégrité des seeds ». Effort ~½ session.
- **Rétrogradation `readyForGlory` = changement de comportement Glory/cascade (à surveiller)** : `assessPillar` rétrograde désormais un pilier à feuille requise vide de COMPLETE → ENRICHED (honnêteté). Conséquence : Glory/cascade attendent le remplissage profond avant de tourner sur ce pilier (correct, mais plus strict qu'avant). Aucune marque canon n'est affectée (0 feuille requise vide). **Déclencheur** : si une marque prod légitimement « en cours » se retrouve bloquée pour Glory sur une feuille qu'on décide finalement optionnelle → reclasser la feuille `.optional()` dans le schema (et non affaiblir l'assessment).
...
- ~~**Détection notoria des CELLULES de matrice partielles (LOW)**~~ — **CLOS (v6.27.322, constat SPAWT « — » partout à 100 % Complet)** : NOUVEAU `findEmptyArrayCellPaths` (pur) rend remplissables les cellules OPTIONNELLES vides d'items EXISTANTS, câblé dans le moteur notoria (SET `arr[i].leaf`) ET l'auto-filler. Garde anti-fabrication STRICTE via `scalarKind` (QUALITATIF only) + passe adversariale v6.27.323 (id/FK/url/preuve/computed exclus). Les deux voies couvrent désormais les cellules de matrice partielles.
- **Cellules d'un SOUS-OBJET imbriqué d'item de matrice (LOW, nouveau résidu v6.27.322)** : `findEmptyArrayCellPaths` descend d'UN niveau dans l'item (clés directes `arr[i].leaf`). Une cellule sous un **sous-objet imbriqué** d'item (`arr[i].sousObjet.feuille`, ex. un `unitEconomics` imbriqué dans un produit) reste au grain objet, laissée à l'opérateur (édition à la main Phase 1). **Honnête aujourd'hui** : quasi aucun schéma d'item n'a de sous-objet imbriqué à feuilles vides sur A/D/V (matrices plates) ; le cas ne s'est pas présenté. **Fermeture** : récursion bornée dans `findEmptyArrayCellPaths` (mêmes gardes scalarKind/REF/evidence appliquées par profondeur). **Déclencheur** : si un item de matrice expose un sous-objet imbriqué que l'opérateur constate vide et non ciblé. Effort ~½ session.
- **Panneau per-pilier : « accepter » ≠ « appliquer » (LOW/MED, pré-existant)** : le panneau recos de `pillar-page.tsx` **accepte** (met en file `ACCEPTED`) mais n'**applique** pas — l'application (avec le gate de remplacement ADR-0090 qui protège réellement) vit au hub Notoria / `applyBatch`. Donc depuis la page pilier, l'opérateur « accepte » mais le pilier n'est pas mis à jour tant qu'il ne passe pas au hub. Le gate protège (une reco RULER_INFERIOR est REJECTED à l'apply), mais le flux à deux temps est **confus** (le nouveau verdict visible atténue la confusion : l'opérateur voit désormais quelles recos n'améliorent pas l'existant avant d'accepter). **Fermeture** : soit brancher un bouton « Appliquer » per-pilier (`applyRecos` après `acceptRecos`, gate inclus, feedback des bloquées), soit clarifier le libellé (« Accepter → à appliquer au hub »). **Déclencheur** : passe UX du panneau recos, ou demande opérateur d'appliquer depuis la page pilier. Effort ~½ session.

## Chantier « remplissage profond ADVE » 2026-07-23 (PR #617) — déférés

- **Glory tool dédié matrice + writeback généralisé (LOW, enhancement)** *(Phase 3b, optionnel)* : Phase 3 fait remplir les matrices par la notoria (auto-filler, toutes ses options de dérivation). Un **Glory tool HYBRID dédié** (`product-value-matrix-architect`, `defineHybridTool`, parité manuelle) donnerait une SECONDE voie (invocable spécifiquement pour la matrice, LLM ou manuel). Il exige un **writeback Glory→pilier généralisé** : aujourd'hui la sortie Glory→pilier est câblée en dur aux 10 champs `D.directionArtistique` (`engine.ts`) — le généraliser (allowlist `{toolSlug→pillarPath}` + `pillar.applyGloryOutputToPath`) débloquerait TOUTE matrice (D/A/E). **Non requis pour la cible** (« la notoria remplit » = l'auto-filler, fait) — pur « option supplémentaire ». **Fermeture** : `defineHybridTool` + `outputSchema` + allowlist writeback + mutation gouvernée. **Déclencheur** : besoin d'une génération matrice tool-based hors auto-filler. Effort ~1 session.
...
- **F5 — tier-gate par-marque** — 🟢 **BÂTI (v6.27.309)** : `checkPaidTier(operatorId, allowedTiers?, strategyId?)` scope désormais par marque quand `strategyId` est fourni — l'abonnement doit être scopé à CETTE marque (`strategyId = <marque>`) OU legacy operator-wide (`strategyId = null` — **filet backward-compat : AUCUN payeur LIVE ne perd l'accès**, tous les subs existants sont `null`) OU multi-marques (`RETAINER_ENTERPRISE`, jamais restreint). 4 callers migrés (`cockpit-router` ×2, `campaign-tracker`, `engine`). God-mode intact (bypass dans `if(!sub)`, la garde ne fait que RÉTRÉCIR la requête). Sans `strategyId` → `where` byte-identique au pré-F5 (operator-wide pur). 13 tests tier-gate (backward-compat / brand-scope deny cross-brand / no-strategyId inchangé / god-mode / ENTERPRISE). `Subscription.strategyId` existait déjà (0 migration).
- **B1 — Q3 non-bypass réellement enforcé** — 🟢 **VÉRIFICATEUR BÂTI** (v6.27.265) : `governed-active-no-new-bypass.test.ts` (HARD) remplace la tautologie du grep — pour CHAQUE routeur `governed-active`, il compte les `.mutation()` NI `governedProcedure` NI `auditedProcedure` NI porteuses d'un `emitIntent*`/`openEmission`, et **gèle ce compte par un baseline décroissant** (78 au 2026-07-22, exact-match) : aucune mutation non gouvernée ne peut PLUS être ajoutée en silence (compte > baseline → merge cassé). **3 routeurs cœur ✅ MIGRÉS (v6.27.307-308, B1 lot par lot)** : migration des vraies mutations métier PENDING vers `governedProcedure` (LEGACY-wrap : `governedProcedure({kind:"LEGACY_...", requireOperator:true}).mutation(body)` — `requireOperator:true` préserve le gate opérateur des `operatorProcedure`, l'ADR-0175 ajoute le brand-scope `canAccessStrategy`). **`notoria` (12)** + **`pillar` (9, previewAmend EXEMPT = lecture LLM zéro-écriture)** + **`campaign-manager` (18)** migrés → **baseline 78 → 39** (`generateReport`/`saveAsTemplate` gagnent en prime une garde `enforceCampaignRawScope` — trou pré-existant fermé ; `confirmInferredField` migré SANS requireOperator = accès fondateur préservé). **Reste (petits routeurs, lots suivants)** : `crm-contacts` (4), `quick-intake` (8, mixte funnel public), `client` (2), `social` (2), `strategy.validateSynthesis` (1), `market-cost` (1), `mission-applications` (1), `monetization` (1, paywall public) + les EXEMPT figés au baseline. **⚠️ Landmine générateur (découverte B1)** : `scripts/generate-legacy-intent-kinds.ts` ne traite QUE les routeurs `lafusee:strangler-active` — or les 3 routeurs cœur sont désormais `governed-active`, donc **AUCUN routeur n'est strangler-active** → re-run du générateur VIDERAIT le bloc AUTOGEN (44+ kinds LEGACY) et casserait tout. Les nouveaux kinds LEGACY sont donc ajoutés À LA MAIN au bloc autogen (frozen de-facto). **Fermeture landmine** : ajouter un shrink-guard au générateur (refuser d'écrire si le nouveau set est vide/plus petit). **Déclencheur** : prochaine touche aux kinds LEGACY / strangler. Le flip du lint `no-direct-service-from-router` à `error` reste secondaire (mécanique distincte — imports de service directs, pas les mutations ; le test HARD est désormais le garde-fou Q3 canonique). **Déclencheur** : chantier « migration governedProcedure des 3 routeurs Neteru cœur ».
...
- **Publication sociale — double-POST sur panne DB TOTALE (LOW extrême)** — ⚖️ **LIMITE PHYSIQUE ACCEPTÉE (purge dette)** : le fix round-16a résout le cas commun (POST réussi + persist échoué → handler sort de PUBLISHING → pas de re-POST). Reste l'arête où le persist ET le fallback échouent tous deux (DB TOTALEMENT indisponible sur 2 écritures consécutives) → re-POST possible. **Irréductible sans clé d'idempotence côté réseau — FB Graph n'en expose PAS** ; un marqueur « posted » persisté AVANT le POST (2-phase) déplace juste la fenêtre (le marqueur peut échouer pareil sous panne totale). Fenêtre extrême (panne DB totale au milliseconde près). **Décision : accepter** — l'ingénierie d'un 2-phase pour une fenêtre de panne-totale n'est pas justifiée. Rouvrir si un doublon réel est observé.
- **Dual-scheduler (daemon + GitHub Actions tous deux ON) — doublons bénins (LOW)** — 🔴 **DÉCISION OPÉRATEUR (purge dette)** : sur la topologie belt-and-suspenders, chaque route tire 2× (le curl GitHub court-circuite le `claimOnce` daemon). Vérifié SANS corruption (`social-sync` claim atomique, `jehuty-refresh` claimOnce, `argos-hunt` upsert déterministe = 1 article, `community-snapshot` fenêtre vélocité = pas d'inflation). Seul reste `anubis-digest`/`lead-nurture` = 1 email dup possible (TOCTOU `sourceHash`, non-fermable par contrainte — cf. entrée sourceHash ci-dessus). **La vraie fermeture est une DÉCISION OPS, pas du code** : choisir UNE topologie en prod — soit le daemon Coolley in-process (`ops-daemon`), soit GitHub Actions `scheduled-ops.yml`, **pas les deux**. Le code est déjà safe dans les deux cas (aucune corruption) ; le dual = juste du gaspillage + le rare doublon d'email.
...
| ~~Relever le conteneur `ollama-embed`~~ — **CLOS le 2026-07-29** (v6.27.371) | Résolu, et la cause n'était **pas** celle que j'avais supposée. Diagnostic réel obtenu via le workflow `ops-ssh` (runner GitHub — egress libre, là où le SSH du bac à sable est refusé) : le conteneur **n'existait pas** (`no such object`), aucune trace d'OOM, 4,5 Go libres. `docker compose up` échouait sur `network q9b4m57yh93gxbjykj470giy declared as external, but could not be found` — **un réseau externe fantôme introduit par mon propre PATCH de compose** quelques heures plus tôt : `compose up` sortait en erreur AVANT toute création, d'où `start`/`restart`/`deploy` sans effet et un statut Coolify périmé. **Solution retenue** : ne pas ressusciter un second daemon sur 2 cœurs — un Ollama tournait déjà (`ollama-internal-…`, up 16 h) **avec `nomic-embed-text` déjà tiré**. Connecté au réseau `coolify` (celui de l'app), `EMBED_SERVICE_URL`/`EMBED_MODEL_NAME` repointés dessus, app **recréée** (`restart` ne suffit pas : l'env d'un conteneur est figé à sa création — seul `deploy` le relit). Sonde : joignable **17 ms**, `provider: ollama`, `dim: 768`. Compose d'`ollama-embed` réparé (réseau fantôme retiré) et service laissé à l'arrêt — redondant |
| ~~Aucune recherche sémantique~~ — **CLOS** | `retrieval: SEMANTIC` vérifié en prod sur SPAWT le 2026-07-29 (200, 2 975 ms, 97/99, 3 documents). Le rattrapage des vecteurs (v6.27.371) a vectorisé **21 fragments, 0 échec** au premier passage d'`ops-sweep` |
| **Marques encore en `DETERMINISTIC`** — Motion19, FanTribe (mesuré 2026-07-29) | Leurs fragments viennent probablement du pool **`SOURCE_CHUNK` legacy** (écrit par `vault-enrichment`), pas de `BRAND_SOURCE` que lit le ranker — c'est la dette « purge des nœuds `SOURCE_CHUNK` legacy » déjà inscrite (ADR-0184). **Sans gravité** : les deux marques répondent 200, citent leur document et **annoncent** leur mode de récupération. Déclencheur de résolution : la purge legacy, ou un re-dépôt du document par la voie `INDEX_BRAND_SOURCE` |
...
- **Meta App Review groupée** (`read_insights`/`instagram_manage_insights`/publish + IG Business Login hors-testeurs) — les scopes marchent DÉJÀ pour les comptes testeurs ; la review = passage public. **Le handler GET `hub.challenge` du webhook temps-réel est 🟢 BÂTI (v6.27.312)** — fail-closed sur `META_WEBHOOK_VERIFY_TOKEN`, comparaison constante-temps, sonde de santé préservée. **Le SEUL reste est l'atome externe pur** : l'App Review Meta accordant l'Advanced Access (le code est drop-in prêt).
...
- **Scoreur A/D/V — endpoints publics 🟢 BÂTIS (v6.27.311)** : `wikipedia.ts` (API REST officielle, ToS-OK, signal axe A notabilité, default-ON) + `search-autocomplete.ts` (endpoint public Google, signal axe D demande, **default-OFF** derrière `SEARCH_AUTOCOMPLETE_ENABLED` car ToS-gray — honnête plutôt que reckless). Anti-fabrication (404 = LIVE-négatif réel, pas un zéro fabriqué), `ConnectorResult<T>` P22-1, gates additifs/monotones (jamais de math de score touchée). **Reste ToS-gated (C, seul un tiers/contrat débloque)** : **Google Trends** (pas d'API officielle) + **scraping d'avis à l'échelle** (`ConnectorResult<T>`, ADR-0149 — dégradation honnête en attendant).
...
- **`/api/chat` — routage LLM Gateway (budget/coût/rate)** — 🟢 **CLOS (2026-07-26, ADR-0179)** : surface streaming `streamChatText` bâtie dans le Gateway (`llm-gateway/streaming.ts`) — cascade providers, parité Sonnet 5 partagée (`parity.ts`), budget Thot, circuit breaker, cost tracking. `/api/chat` réécrite pour passer par le conseil de marque (`askCouncilStream`) → plus aucun appel provider direct. Pre-flight `isTextLLMAvailable` + rate-limit par user + historique persisté (ADR-0181). Le mismatch de protocole de stream (client v3/v4 vs route texte brut) qui cassait le chat est réparé.
- **Lecture d'image — refus honnête, pas capacité (2026-07-28)** : `extractImage` fabriquait (200 caractères d'en-tête base64 envoyés à un modèle **texte**, description « détaillée » inventée puis stockée en `rawContent`). Remplacé par un refus explicite avec le geste à faire. **Rétablir une vraie lecture d'image demande une surface vision au LLM Gateway** (`callLLM` ne transporte pas d'image) + un provider qui la supporte : c'est un chantier Gateway, pas un correctif. **Déclencheur** : besoin réel d'ingérer des chartes en image (aujourd'hui le PDF couvre le cas). Effort ~1 session + vérification provider. En attendant, le refus est la bonne réponse — mieux vaut ne rien écrire qu'inventer une description qui deviendrait ensuite une « source » citable.
- **Ancrage documentaire — déférés (2026-07-28, [ADR-0184](adr/0184-source-grounding-measured-not-claimed.md))** : la mesure d'ancrage est posée et **advisory** (elle informe, elle ne refuse pas — doctrine ADR-0085). Restent : (1) **passage en BLOCK** après période d'observation (même trajectoire que la gate C6 brief↔ADVE, ADR-0103 — une reco `UNGROUNDED` sur une marque documentée serait refusée, avec override opérateur explicite) ; (2) **rang 2 de la cascade d'ancrage** — 🟢 **BÂTI** ([ADR-0185](adr/0185-composed-brand-bible-rank-2-anchor.md), v6.27.358) : le livre de marque composé existe (vue HTML + PDF, déterministe). Reste à le *consommer* comme repli explicite dans la mesure d'ancrage elle-même — aujourd'hui il fait référence pour le lecteur humain, pas encore pour `computeRecoGrounding` ; (3) **purge des nœuds `SOURCE_CHUNK` legacy** — plus aucun écrivain, encore lus en récupération ; une purge est possible une fois toutes les sources ré-indexées en `BRAND_SOURCE` (sans urgence : le double stockage ne fausse rien, il coûte). **Déclencheur** : (1) après ~2 semaines d'observation en prod ; (2) chantier Brand Book ; (3) hygiène base. Effort ~½ session chacun.
- **Conseil de marque — déférés (2026-07-26, ADR-0180)** : (1) **minutes de délibération persistées** — `deliberate()` retourne le résultat mais ne persiste rien ; un `BrandAsset kind=COUNCIL_MINUTES` via Intent MESTOR permettrait de garder la trace des analyses. (2) **UI cockpit « Analyse approfondie »** — la délibération n'est exposée qu'en `operatorProcedure` + tool MCP ; une surface cockpit founder avec progression NSP (pattern `oracle-section/stream-events`) reste à bâtir. (3) **consultation d'experts dans le tour interactif** — v1 = coordinateur seul en chat (latence) ; un mode « chat profond » consultant les experts est déféré. **Déclencheur** : demande opérateur d'archiver les délibérations, ou passe UX cockpit conseil. Effort ~1 session chacun.
- **MCP — resources[] (2026-07-26, ADR-0182)** : les **28 `resources[]`** déclarées dans les modules MCP (ex. `intelligence://pillars/{strategyId}`) ne sont câblées à AUCUN transport — le scoping BRAND des URIs de resource n'est pas trivial (pas de champ `strategyId` structuré comme sur les tools). Câblage = mapper chaque URI → une lecture scopée + exposer via `resources/list`+`resources/read` du serveur MCP. **Déclencheur** : besoin d'exposer des ressources en lecture pure côté client MCP. Effort ~1 session. *(L'OAuth 2.1 pour claude.ai — anciennement listé ici — est **CLOS** : ADR-0183, serveur d'autorisation complet livré 2026-07-26.)*
- **MCP OAuth — polish (2026-07-26, ADR-0183)** : le serveur d'autorisation est livré et fonctionnel. Restent des raffinements non-bloquants : (1) **révocation explicite** d'un token OAuth depuis la console (aujourd'hui : expiration naturelle + rotation du refresh) ; (2) **purge cron** des `McpOAuthCode`/`McpOAuthToken` expirés (accumulation lente, sans risque) ; (3) consentement **multi-marques** en un token (v1 = une portée par token ; ré-autoriser pour changer de marque). **Déclencheur** : passe console MCP, ou si les tables OAuth grossissent en prod. Effort ~½ session chacun.
