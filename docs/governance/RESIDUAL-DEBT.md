...
- **Ingestion externe générique + Brevo pull (PR-B, ADR-0146 à créer)** : la « remontée automatique » des chiffres (quiz/app/CRM push + Brevo pull) n'est pas encore construite — les sources autres que Réseaux/Email s'affichent honnêtement « à connecter — bientôt ». Chantier : `INGEST_EXTERNAL_METRIC` (governor ANUBIS) + `POST /api/ingest/metrics` (token par stratégie via `MediaPlatformConnection`) + cron `metrics-pull` Brevo — tout agnostique.
