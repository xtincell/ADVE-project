...
import { CalendarRange, Wand2, Loader2, CalendarClock, X, Inbox, Rocket, CheckCircle2, ArrowRight } from "lucide-react";
...
export function RoadmapCalendarPanel() {
  ...
  function ActionRow({ a }: { a: (typeof actions)[number] }) {
    ...
    const budgetRange = a.budgetMin || a.budgetMax
      ? `${a.budgetMin?.toLocaleString("fr-FR") ?? "0"} - ${a.budgetMax?.toLocaleString("fr-FR") ?? "—"} XAF`
      : "";
    const isExpanded = expandedId === a.id;
    const isAccepted = a.status === "ACCEPTED";

    const generateMut = trpc.strategy.generateProjectsFromActions.useMutation({
      onSuccess: () => {
        refetch();
      },
    });
    // Lien vers l'étape suivante : la campagne créée (sinon le pipeline campagnes).
    const freshCampaignId = generateMut.data?.projects?.[0]?.campaignId;
    const campaignHref = freshCampaignId
      ? `/cockpit/operate/campaigns/${freshCampaignId}`
    ...
    return (
      <div className="flex w-full min-w-0 flex-col rounded-lg border border-white/5 bg-surface-raised transition-all hover:border-white/10">
        ...
        >
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2 min-w-0">
              <span
                title={a.title}
                className={`truncate text-xs font-semibold ${isExpanded ? "text-accent" : "text-foreground"}`}
              >
                {a.title}
              </span>
              {isAccepted && (
            ...
            {!isExpanded && a.description ? (
              <p className="mt-0.5 truncate text-[10px] text-foreground-muted">{a.description}</p>
            ) : null}
            <div className="mt-1 flex flex-wrap items-center gap-1">
              ...
              {budgetRange ? <span className="rounded bg-white/5 px-1.5 py-0.5 text-[9px] text-foreground-muted">{fcfa(a.budgetMax ?? a.budgetMin)} FCFA</span> : null}
        ...
        {isExpanded && (
          <div className="border-t border-white/5 bg-background/30 p-3 text-xs space-y-3">
            {a.description && (
              <div>
                <h5 className="font-bold text-foreground-muted uppercase text-[9px] tracking-wide mb-1">Description</h5>
                <p className="text-foreground-secondary leading-relaxed">{a.description}</p>
              </div>
            )}
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
              {a.touchpoint && (
                <div>
                  <h5 className="font-bold text-foreground-muted uppercase text-[9px] tracking-wide">Touchpoint</h5>
                  <p className="text-foreground">{a.touchpoint}</p>
                </div>
              )}
              {a.aarrrIntent && (
                <div>
                  <h5 className="font-bold text-foreground-muted uppercase text-[9px] tracking-wide">Intention AARRR</h5>
                  <p className="text-foreground">{a.aarrrIntent}</p>
                </div>
              )}
              {a.persona && (
                <div>
                  <h5 className="font-bold text-foreground-muted uppercase text-[9px] tracking-wide">Persona</h5>
                  <p className="text-foreground">{a.persona}</p>
                </div>
              )}
              {a.locality && (
                <div>
                  <h5 className="font-bold text-foreground-muted uppercase text-[9px] tracking-wide">Localité</h5>
                  <p className="text-foreground">{a.locality}</p>
                </div>
              )}
              {budgetRange && (
                <div>
                  <h5 className="font-bold text-foreground-muted uppercase text-[9px] tracking-wide">Budget</h5>
                  <p className="text-foreground">{budgetRange}</p>
                </div>
              )}
              {a.sku && (
                <div>
                  <h5 className="font-bold text-foreground-muted uppercase text-[9px] tracking-wide">SKU / Produit</h5>
                  <p className="text-foreground">{a.sku}</p>
                </div>
              )}
            </div>
            ...
            <div className="flex flex-col items-end gap-2 pt-2 border-t border-white/5">
              {isAccepted ? (
                <div className="flex w-full flex-col items-end gap-1.5">
                  ...
                  <Link
                    href={campaignHref}
                    className="flex items-center gap-1 rounded bg-accent px-3 py-1.5 text-xs font-semibold text-accent-foreground transition-opacity hover:opacity-90"
                  >
                    Voir la campagne & production
                ...
                <>
                  {generateMut.isError ? (
                    <p className="w-full text-right text-[10px] font-medium text-error">
                      Échec : {generateMut.error.message}
                    </p>
                  ) : null}
                  <button
                    type="button"
                    disabled={generateMut.isPending}
                    onClick={() => generateMut.mutate({ strategyId: sid, actionIds: [a.id] })}
