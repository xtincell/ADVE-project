...
Déclarés dans `src/styles/tokens/domain.css`. Catalogue : [design-tokens/domain.md](design-tokens/domain.md). Cohérents avec `BRAINS` const ([manifest.ts:23](../../src/server/governance/manifest.ts)) — **7 Neteru actifs** : Mestor / Artemis / Seshat / Thot / Ptah / Imhotep / Anubis.
...
@theme {
  ...
  /* Imhotep + Anubis activés Phase 7+/8+ — tokens à provisionner si UI dédiée nécessaire */
...
- [PANTHEON.md](PANTHEON.md) — 7 Neteru actifs (septuor mai 2026)
