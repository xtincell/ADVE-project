...
/**
 * Rattache un SuperfanProfile à sa personne — l'écriture `personId` vit ICI
 * (single-writer HARD) ; superfan-ingest appelle ce helper, jamais l'update brut.
 */
export async function linkSuperfanProfileToPerson(
  client: IdentityDbClient,
  profileId: string,
  personId: string,
): Promise<void> {
  await client.superfanProfile.update({
    where: { id: profileId },
    data: { personId },
  });
}
