...
| **1** | PROD | **Verrou de sortie** (+ entrée sur les sites qui contournent les chokepoints LOT 0). Sous-lots : **1a** points d'intake ✅ (`deduce-adve`, `boot-sequence`, `narrate-adve`, `rtis-draft`, `brief-ingest`) ; **1b** rtis-cascade/notoria/mestor-insights ✅ (entrée) ; **1c** Glory (**20/37 faits** — 12 CR + 4 DC + 4 BRAND ; schémas de sortie *réels* dérivés des promptTemplate, `glory-output-schemas.ts` ; baseline gate ratchetée 37→17 ; restent BRAND forge-couplés, LSI, AdOps `adops-tools.ts`, phase14 `imhotep-tools.ts`, à traiter avec vérif fonctionnelle) ; **1d** Glory brand ; **1e** campaign-plan/vault-enrichment/reliquat. *(Note : les ~55 Glory tools 1c/1d ont déjà l'entrée durcie via LOT 0/`engine.ts` ; leur reste = schéma de sortie. Frameworks 28/28 déjà OK.)* | 🟡 1a+1b faits |
