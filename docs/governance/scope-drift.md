...
| 2026-06-21 | #280 | chore(ci) Golden Path E2E auto-diagnostiquable (imprime le détail des findings en échec) | NEFER (auto) — opérateur | Outillage CI (diagnostic du test golden-path), hors phases 0–9 (même nature que #79/#278). **0 code de production touché**, 0 nouveau Neter, 0 model Prisma, 0 bypass. Révèle au prochain run le finding `1-landing` pré-existant pour le corriger à la racine. Cf. Justification — out-of-scope dans le body PR. |
