...
| **1** | PROD | **Verrou de sortie** (+ entrée sur les sites qui contournent les chokepoints LOT 0). Sous-lots : **1a** points d'intake ✅ (`deduce-adve`, `boot-sequence`, `narrate-adve`, `rtis-draft`, `brief-ingest`) ; **1b** rtis-cascade/notoria/mestor-insights ✅ (entrée) ; **1c** Glory (**11/37 faits** — 7 CR + 4 DC ; schémas de sortie *réels* dérivés des promptTemplate, `glory-output-schemas.ts` ; baseline gate ratchetée 37→26) ; **1d** Glory brand ; **1e** campaign-plan/vault-enrichment/reliquat. *(Note : les ~55 Glory tools 1c/1d ont déjà l'entrée durcie via LOT 0/`engine.ts` ; leur reste = schéma de sortie. Frameworks 28/28 déjà OK.)* | 🟡 1a+1b faits |
