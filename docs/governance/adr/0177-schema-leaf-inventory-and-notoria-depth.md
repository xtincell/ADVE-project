...
- **Tests anti-drift** : `schema-leaf-paths.test.ts` (inventaire + vides + garde union-string + exclusion NEEDS_HUMAN + exclusion COMPLETE_OPTIONAL + union `array|object` en forme tableau) et `reco-apply-nested.test.ts` (SET/EXTEND/ADD/MODIFY profonds, parité top-level, base non mutée, proto-guard uniforme skip+warning, EXTEND profond préserve les frères, chevauchement parent/enfant surfacé).
- **Passe adversariale (v6.27.321)** : 5 reviewers indépendants (un par surface, repro `tsx`) ont trouvé 7 défauts réels — 1 fabrication (`v.productSystem` non exclu par `findEmptyLeafPaths`), 1 destruction silencieuse (EXTEND-par-grand-parent écrasant un frère rempli), 1 faux vide union `array|object`, 1 incohérence financière (ratios calculables → LLM), 1 garde de prototype non uniforme, + 2 pré-existants (justification `explain` jamais rendue, valeur BLOCK financière persistée). Tous réparés avec tests. `findEmptyLeafPaths` honore désormais les DEUX exclusions du contrat (NEEDS_HUMAN + COMPLETE_OPTIONAL) et la forme tableau des unions ; le groupement notoria EXTEND vise le parent immédiat ; la garde de prototype est uniforme.
