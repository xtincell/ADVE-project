...
- **Propriété des marques — MÉCANISME LIVRÉ, exécution PROD à faire**. Les deux voies de provisionnement ne savaient produire que des **collaborateurs délégués** (scopés par le firewall de zones ADR-0131) : les dirigeants à qui l'on remettait leur marque sont restés les délégués de leur propre bien. Trois mécanismes referment l'écart : `attachAs: "OWNER"` à la création, `accounts.transferBrandOwnership` pour un compte **déjà existant** (que la création refuse par construction — c'est le cas de Lionel), et `/api/admin/prod-finish` qui n'abandonne plus le transfert quand un mot de passe existe déjà. **À exécuter contre la base Coolify** (aucune base locale ici — `P1001`) : `npm run transfer:owner -- lionel@motion19.cm motion19` (confirmé opérateur : il EST le propriétaire) et, pour Hilaire, `npm run transfer:owner -- jlombat@gmail.com fantribe` **une fois la marque Fantribe créée en prod** — elle n'existe pas encore. Les deux scripts prouvent 5 invariants au lieu de les affirmer.
