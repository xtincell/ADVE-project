...
- **IDOR round-2 — ops keyées sur un id d'ENTITÉ (hors strategyId/campaignId)** *(audit adversarial « TOUT » round-2, 2026-07-22)* : les gardes ADR-0166/0175 + le nouveau `campaignScopedProcedure` couvrent `strategyId`/`campaignId` de tête. **CRITIQUE fermé** (v6.27.269-271 : CRM any-account, Credentials Vault, brand-bible PDF, vault promote/reject, PR PII, signals, jehuty, campaign-manager campaignId reads+writes). **Restant (MEDIUM, résolution entité→marque à ajouter)** : ~24 procédures campaign-manager keyées sur un id d'entité SANS `campaignId` (`updateAction`/`updateExecution`/`transitionExecution`/`updateAmplification`/`deleteAmplification`/`updateMilestone`/`completeMilestone`/`deleteMilestone`/`updateBudgetLine`/`deleteBudgetLine`/`decideApproval`/`versionAsset`/`publishAssetToBrandVault`/`updateBrief`/`getReport`/`addDependency`/`getFieldOp`/`updateFieldOp`/`deleteFieldOp`/`getFieldReport`/`validateFieldReport`/`rejectFieldReport`/`getPendingApprovals`/`briefStatusMany`) + `deliverable-tracking` (`addSignal`/`getByDeliverable`/`getImpact` — métadonnées faible valeur). **Plan** : pour chacune, charger l'entité → remonter `campaignId`/`strategyId` → `enforceCampaignAccess`/`enforceStrategyAccess` (pattern posé). **Effort** : ~½ session. **Déclencheur** : prochaine passe sécurité. **Verrou à bâtir** : étendre `strategy-ownership-guard.test.ts` aux ids d'entité (le scanner actuel ne couvre que `strategyId` de tête — les statiques `getActionTypes`/`getBriefTypes`/`recommendActions`/`scoreActionFit` sont exempts).
- **CRM — ownership deal↔opérateur par-deal + `getConversionMetrics` scope** *(round-2)* : `crm.*` est désormais operator-gated + lectures agrégées scopées (v6.27.270), mais les ops keyées `dealId` (`updateDeal`/`advanceDeal`/`getDeal`…) ne vérifient pas que le deal appartient à l'opérateur appelant (cross-opérateur ; théorique en mono-opérateur UPgraders), et `getConversionMetrics` (funnelMapping agrégé) reste global. **Plan** : résoudre `deal.strategy.operatorId` == appelant + scope funnelMapping. **Déclencheur** : multi-opérateur réel.
