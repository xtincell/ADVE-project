...
- **Restants round-6 (tracés)** : **(a) entité-id DANS le MCP** — `scopeMcpParams` scope `strategyId` ; un outil MCP prenant un id d'ENTITÉ (signalId/pillarId d'une autre marque) via une clé BRAND n'est pas encore résolu (même classe entité-id, côté MCP) → per-tool resolution à ajouter. **(b) scoping PAR-opérateur** des reads operatorId (les gates round-6 sont operator/admin = STAFF, pas `operatorId==caller`) + `anubis.fetchDeliveryReport` utilise les creds d'un autre opérateur (à scoper broadcastJob→operatorId). **(c) `media-buying.syncToCampaign`** (`campaignId` leurre non vérifié — LOW). **(d) `phase18-residuals.*` gouvernance** : `adminProcedure` ferme l'authz mais reste ungoverned (audit-gap). **(e) test bypass structurel** : `governed-active-no-new-bypass` ne scanne QUE les routeurs marqués `lafusee:governed-active` — l'étendre à TOUT routeur avec `.mutation(` (payment/newsletter/blog/error-vault/auth peuvent ajouter une mutation ungoverned sans signal CI). **Déclencheur** : prochaine passe sécurité / multi-opérateur réel.
