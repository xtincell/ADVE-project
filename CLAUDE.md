...
- **Phase 19** (Campaign tracker L2 Instrumental — double-layer canonical, [ADR-0052](docs/governance/adr/0052-campaign-module-canonical-trajectory-instrument.md) v2) — 🟡 partial-shipped (Vague 1 = Cluster A + B). Module Campaign upgrade en double-couche : L1 Operational (existant inchangé) + L2 Instrumental (lecture composée orchestrée cross-Neteru). Vague 1 : 6 Intent kinds + service `campaign-tracker/` skeleton (governor MESTOR, dispatcher cross-Neteru) + extensions Prisma `Campaign` (snapshots immutables) et `CampaignAction` (manipulationModeApplied + bigIdeaCoherenceScore) + tests anti-drift. **Cap APOGEE 7/7 préservé** — pas de nouveau Neter. Trois primitives architecturales OS-natives mobilisées (Capability flags 4-états READY/PARTIAL/STUB/DISABLED + STUB→MVP→PRODUCTION par sous-cluster + double-layer L1/L2). Vague 2 (Cluster C + D) et Vague 3 (Cluster E + F + G + H) restent pending — cf. [RESIDUAL-DEBT.md](docs/governance/RESIDUAL-DEBT.md) §Phase 19.
