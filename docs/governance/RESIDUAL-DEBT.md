...
- **Compte d'Hilaire (`jlombat@gmail.com`) sur Fantribe — CODE PRÊT, exécution PROD à faire**. L'opérateur a précisé qu'il est **propriétaire** de la marque, comme Stéphanie sur SPAWT. Or `createBrandLogin` ne savait produire QUE des collaborateurs délégués (scopés par le firewall de zones ADR-0131) : mode `attachAs: "OWNER"` ajouté (pose `Strategy.userId`, motif Stéphanie, l'ancien propriétaire devient DIGITAL_DIRECTOR — personne ne perd son accès), exposé au formulaire console, et `npm run verify:brand-owner` prouve les 6 invariants (mot de passe vérifiable · propriétaire · cockpit scopé · firewall qui ne le vise pas · aucune autre marque visible · mot de passe absent de l'émission). **Reste à faire, non faisable d'ici** : (1) la marque **Fantribe doit exister** en base de prod ; (2) exécuter le rattachement contre Coolify (pas de base locale — `P1001` sur `qosouizh7…`). **À vérifier aussi** : Lionel/Motion19 a été provisionné par l'ancien chemin, donc en **collaborateur**, pas en propriétaire — si l'intention était la propriété, il faut le repasser en `OWNER`.
