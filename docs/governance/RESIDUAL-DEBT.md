...
- **Dé-densification campagne `[id]`** (~2 270 l. / 12 onglets) — exige des décisions produit sur la fusion des onglets, pas un sweep mécanique.
...
- **Purge vocab surfaces opérateur-gated** : `apogee-maintenance` (« Loi 4 APOGEE ») + `brand/rtis(+synthese)` restent en allowlist justifiée du test vocabulaire — purger ou supprimer ces routes à l'occasion.
- **`window.prompt()`** dans `canon-campaigns-panel.tsx` (titre de campagne ponctuelle) — hors décompte T4 de l'audit ; à convertir en Modal+input.
- **Alignement pricing/Console sur ADR-0123** : `pricing-tiers.ts` affiche « Rapport ADVE+RTIS » sur `/pricing` (funnel public, hors périmètre cockpit) — aligner au prochain chantier pricing.
- **i18n hors nav** : les corps de pages cockpit restent FR-littéraux (clés i18n non généralisées hors nav/piliers).
- **Fuite de résidu `enrichOracle` legacy** : la cohabitation ADR-0073 est désormais invisible du founder (opérateur-only) ; la dépose du chemin legacy lui-même reste conditionnée à l'audit de couverture du panel progressif (drift D-6.3).
