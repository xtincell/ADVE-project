...
Vague [ADR-0126](adr/0126-market-scale-aware-scoring.md) shippée (échelle déclarée + évidence scale-aware + fixes cult-index + référentiel affiché). Restes tranchés hors vague, par gravité :

- **Overton par polity** : `Sector.slug @unique` = un seul axe culturel GLOBAL par secteur — déplacer la fenêtre d'un quartier ≠ d'une nation ≠ d'un continent. Fermer = re-clé `Sector` (secteur × portée géo), harvesting Tarsis par polity, backfill des axes existants, pondération CULTE/ICONE par largeur de fenêtre déplacée. Chantier propre (~1 sprint), ADR séparé.
- **Writer gouverné `SuperfanProfile`** : aucun chemin applicatif ne crée ces rows (seeds/DB uniquement). Toute future ingestion (CRM connector Phase 23) DOIT passer par un Intent gouverné — sinon vecteur d'inflation d'évidence (un monopole ingère sa base client → « superfans » gratuits). À poser AVANT de brancher le premier writer.
- **UI d'édition des 3 champs déclarés** (`marketScale`/`addressableAudience`/`brandFoundedYear`) : réglages Strategy (console opérateur + cockpit founder en déclaratif). Aujourd'hui éditables uniquement via les mutations strategy existantes.
- **Leaderboard/Argos segmentés par échelle** + **référentiel d'échelle dans l'EFR contractuel** (l'obligation d'effet promet un palier — sans référentiel déclaré au contrat, flou juridique).
- **Recalibrage des snapshots cult historiques** : les `CultIndexSnapshot` pré-fix (dimensions saturées par le bug d'unités) restent immuables (Loi 1) — les comparaisons temporelles traversant le 2026-07-11 doivent le savoir (annotation UI possible).
