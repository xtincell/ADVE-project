...
- **Yggdrasil** ([ADR-0082](docs/governance/adr/0082-yggdrasil-value-circulation-substrate.md)) — substrat de circulation de la valeur, **gouverné par Mestor**. 3 invariants : Q1 traçabilité via `IntentEmission.id` hash-chained / Q2 observabilité via `NspEvent` / Q3 gouvernance via gate Mestor.
...
- **Phase 23 — Câblage des mécaniques pivot mission (superfans × Overton) MVP→PRODUCTION** — 📋 **EPICS_DRAFTED, READY FOR IMPLEMENTATION**. Closure-roadmap target #1 (anciennement Phase 22 pré-rename 2026-05-15). PRD + UX + architecture + epics livrés (`_bmad-output/planning-artifacts/`). 9 décisions D1-D9 + 7 patterns P22-1..7 + 5 child ADRs réservés 0077-0081 + **53 stories réparties en 7 epics** (Foundations → Connectors → Overton → Superfan → Glory tools HYBRID → Calibration + Lifecycle → Cockpit + Closure). Owning Neteru : **Seshat · Anubis · Artemis** (Ptah dropped par architecture step-02 — pas de scope forge). Surface réelle (après correction PRD scope par architecture D1) : 2 connector façades Tarsis API + CRM via Credentials Vault + 1 lifecycle Intent paramétré `PROMOTE_PIVOT_SUBCLUSTER` + ML calibration pure-TS sans dépendance + manual-first parity HARD-test + 1 Cockpit route mountant `<OvertonRadar>` existant. 0 new Prisma model. Cap APOGEE 7/7 préservé. Étape suivante : implementation Epic 1 (Governance Foundations) — voir `_bmad-output/planning-artifacts/epics.md`.
- **Phase 30 — Yggdrasil canonization** ([ADR-0082](docs/governance/adr/0082-yggdrasil-value-circulation-substrate.md), 2026-05-15) — ✅ **doc-shipped** (canonization du substrat). Yggdrasil = substrat canonique de circulation de la valeur dans La Fusée OS, gouverné par **Mestor** (Guidance §4.2). 3 invariants (Q1 traçabilité / Q2 observabilité / Q3 gouvernance). **Pas un Neter** — substrat comme NSP ou la layering cascade. Cap APOGEE 7/7 préservé. Propagation 7 sources : LEXICON entrée `YGGDRASIL` + PANTHEON §7 Substrats + APOGEE §4.2 + cette section + (BRAINS const + Governor type inchangés). Follow-up Phase 30-bis : anti-drift test `yggdrasil-three-invariants.test.ts` (soft baseline → HARD après stress-test 1 mois). Aucun code touché par la canonization.
