  ...
  return {
    mckinsey7s: {
      ...
      shared_values: dim(
        ...
        "Valeurs de marque non déclarées",
        "Renseignez vos valeurs de marque (minimum 3, avec justification)",
      ),
      style: dim(
        personnalite ? `Ton de voix : ${personnalite}` : null,
        [personnalite],
        "Ton de voix non défini",
        "Renseignez votre ton de voix dans le pilier Distinction",
      ),
      staff: dim(
        equipe.length > 0 ? `${equipe.length} membre(s) clé(s) — ${ctx.superfanCount} superfan(s) trackés en orbite` : null,
        [equipe.length > 0 ? equipe : null],
        "Effectifs non déclarés",
        "Complétez votre équipe et activez le programme équipe",
      ),
      skills: dim(
        ...
        "Détaillez les compétences de chaque profil de l'équipe dirigeante",
      ),
    },
  };
}
