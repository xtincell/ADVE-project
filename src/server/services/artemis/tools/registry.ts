...
// ─── Types ───────────────────────────────────────────────────────────────────
// Types Glory + `defineHybridTool` extraits dans ./tool-types (leaf) — rompt
// les cycles registry ⇄ {adops,higgsfield,market-research,phase13-oracle,
// phase14-imhotep,phase15-anubis,phase19}-tools (madge). Réexportés ici.
export type {
  GloryLayer,
  GloryExecutionType,
  GloryToolMcpDescriptor,
  GloryToolStatus,
  GloryToolNature,
  PillarPath,
...
JSON : { "prompts": [{ "format", "prompt", "style_notes", "copy_overlay" }] }`,
    status: "ACTIVE",
    // Phase 9 (ADR-0009) — brief-to-forge : auto-handoff vers Ptah avec Nano Banana Pro
    forgeOutput: {
      forgeKind: "image",
      providerHint: "magnific",
      modelHint: "nano-banana-pro",
