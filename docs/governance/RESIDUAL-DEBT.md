...
- **Compte d'Hilaire sur la marque Fantribe — BLOQUÉ (opérateur)**. La feature existe (`createBrandLogin`, ADR-0140) ; il manque **l'e-mail d'Hilaire et son rôle métier** (`CampaignTeamRole`, qui détermine ses zones d'écriture — ADR-0131). C'est une écriture en base de PROD : à exécuter contre Coolify, pas en local. **Déclencheur** : l'opérateur fournit e-mail + rôle.
