...
export function PlateformeStrategique({ data }: Props) {
  return (
    <div className="space-y-6">
      ...
      <div>
        <h3 className="mb-3 text-sm font-semibold uppercase tracking-wider text-zinc-400">Architecture strategique</h3>
      ...
      {data.ikigai && (
        <div>
          <h3 className="mb-3 text-sm font-semibold uppercase tracking-wider text-zinc-400">Ikigai de marque</h3>
      ...
      {data.valeurs.length > 0 && (
        <div>
          <h3 className="mb-3 text-sm font-semibold uppercase tracking-wider text-zinc-400">Valeurs (Schwartz)</h3>
          <div className="space-y-2">
            {data.valeurs.sort((a, b) => a.rang - b.rang).map((v, i) => (
              <div key={i} className="flex items-start gap-3 rounded-lg border border-zinc-800 bg-zinc-900/50 p-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-zinc-200">{v.valeur}</p>
                  {v.justification && <p className="mt-0.5 text-xs text-zinc-500">{v.justification}</p>}
                </div>
              </div>
            ))}
          </div>
      ...
      {data.tonDeVoix && (
        <div>
          <h3 className="mb-3 text-sm font-semibold uppercase tracking-wider text-zinc-400">Ton de voix</h3>
          <div className="grid gap-4 sm:grid-cols-3">
            {data.tonDeVoix.personnalite.length > 0 && (
              <div>
                <p className="mb-1 text-xs text-zinc-600">Personnalite</p>
                <div className="flex flex-wrap gap-1">
                  {data.tonDeVoix.personnalite.map((t, i) => (
                    <span key={i} className="rounded-full bg-zinc-800 px-2 py-0.5 text-xs text-zinc-300">{t}</span>
                  ))}
                </div>
              </div>
            )}
            {data.tonDeVoix.onDit.length > 0 && (
              <div>
                ...
                {data.tonDeVoix.onDit.map((t, i) => <p key={i} className="text-xs text-zinc-400">+ {t}</p>)}
              </div>
            )}
            {data.tonDeVoix.onNeDitPas.length > 0 && (
              <div>
                <p className="mb-1 text-xs text-red-400">On ne dit pas</p>
                {data.tonDeVoix.onNeDitPas.map((t, i) => <p key={i} className="text-xs text-zinc-400">- {t}</p>)}
              </div>
            )}
          </div>
        </div>
      ...
      {data.assetsLinguistiques && (
        <div>
          <h3 className="mb-3 text-sm font-semibold uppercase tracking-wider text-zinc-400">Assets linguistiques</h3>
          <div className="space-y-2">
      ...
      {data.messagingFramework.length > 0 && (
        <div>
          <h3 className="mb-3 text-sm font-semibold uppercase tracking-wider text-zinc-400">Messaging framework</h3>
          <DataTable
...
function ArchRow({ label, value, italic, accent }: { label: string; value: string; italic?: boolean; accent?: boolean }) {
  return (
    <div className="flex items-start gap-4 rounded-lg border border-zinc-800/50 bg-zinc-900/30 px-4 py-3">
      <span className="w-40 shrink-0 text-xs font-semibold uppercase tracking-wider text-zinc-500">{label}</span>
      <span className={`text-sm ${accent ? "font-semibold text-orange-400" : italic ? "italic text-zinc-500" : "text-zinc-300"}`}>
        {value}
      </span>
    </div>
  );
...
function IkigaiQuadrant({ label, value, color }: { label: string; value: string; color: string }) {
  return (
    <div className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-4" style={{ borderLeftColor: color, borderLeftWidth: 3 }}>
