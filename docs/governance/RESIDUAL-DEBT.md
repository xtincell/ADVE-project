...
- **Cohérence de palier (ADR-0167) — partielle (MED)**. `effectiveTier` (palier officiel persisté `Strategy.apogeeTier`, sinon dérivé) est honoré par le cockpit et le MCP, mais une douzaine de surfaces dérivent encore le palier du seul score via `classifyTier` — dont `strategy.getWithScore`, le sélecteur de marque, le portefeuille console et le **widget public** `/api/widget/score`. Conséquence : après une promotion gouvernée suivie d'une baisse de score, ces surfaces affichent l'ancien palier pendant que les autres affichent le nouveau — la Loi 1 « pas de régression silencieuse » ne tient que sur une partie des écrans. Le verrou CI n'interdit `classifyTier` que sous `src/server/mcp`. **Fermeture** : router ces surfaces sur `effectiveTier` + élargir le verrou. **Déclencheur** : première promotion de palier en prod. Effort ~½ session.
...
| **Relever le conteneur `ollama-embed`** (service Coolify `l131k79kxl7lyewonqv7jbxp`, statut `exited`) — **bloqué sur l'accès hôte** | v6.27.369. Historique du 2026-07-29 : la cause d'origine était un **rattachement réseau** (le service portait `destination_id = 0` et ne joignait aucun réseau partagé) ; corrigé, la sonde `/api/admin/embed-check` a répondu `provider: ollama`, `dim: 768`, **854 ms**. Une passe d'indexation en masse (5 documents lancés ensemble) sur un VPS **2 cœurs** l'a ensuite saturé, puis le conteneur est sorti. `start` et `restart` via l'API Coolify sont **acceptés (« queued ») mais sans effet** — statut `exited` constant sur 3 min de sondage. Diagnostic impossible d'ici : l'API Coolify **n'expose pas** de route `logs` pour un *service* (404, contrairement aux *applications*), et le SSH est refusé au niveau bannière (`Connection reset by peer`, proxy du bac à sable sain — donc côté VPS, probablement un plafond de connexions après les nombreuses sessions du jour). **Tenté et épuisé depuis le bac à sable** : `start` ×3, `restart` ×1, et un **PATCH du `docker_compose_raw`** ajoutant le bridage `OLLAMA_NUM_PARALLEL=1` · `OLLAMA_MAX_LOADED_MODELS=1` · `OLLAMA_KEEP_ALIVE=10m` (appliqué, réversible — compose précédent sauvegardé ; il prendra effet au premier démarrage réussi). Statut `exited` inchangé sur ~45 min. **Action opérateur** : ouvrir les journaux du conteneur (`docker logs ollama-embed`, chercher un OOM — c'est une **piste**, pas un diagnostic établi) ; à défaut, déporter l'embedding sur une clé (`OPENROUTER_API_KEY`, le moins cher) puis redémarrer la ressource — une variable n'est lue qu'au boot |
| **Conséquence produit tant que ci-dessus n'est pas fait** — aucune recherche sémantique | **Ne casse rien, et le dit** : la cascade dégrade en vecteurs vides (ADR-0108 sur les deux jambes), aucun appel ne pend plus (`EMBED_TIMEOUT_MS`, v6.27.369) et le mode de récupération est annoncé à l'écran. Vérifié en prod le 2026-07-29 **service éteint** : `brandBible.get` → 200 en **1 051 ms**, `retrieval: DETERMINISTIC`, 97/99 éléments, 3 documents cités. Mais livre de marque, conseil de marque, `rag_search` MCP et contexte source de la Notoria tournent tous en repli par **recouvrement de termes** |
