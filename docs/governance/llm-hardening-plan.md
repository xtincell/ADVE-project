...
| **1** | PROD | **Verrou de sortie** (+ entrée sur les sites qui contournent les chokepoints LOT 0). Sous-lots : **1a** points d'intake (`deduce-adve`, `boot-sequence` faits ; `narrate-adve`/`rtis-draft`/`brief-ingest` à venir) ; **1b** rtis-cascade/notoria/mestor-insights ; **1c** Glory concept ; **1d** Glory brand ; **1e** campaign-plan/vault-enrichment/reliquat. | 🟡 1a partiel |
