# ADR-0053 — LLM as UI orchestrator (manual-first parity invariant)

**Status** : Accepted
**Date** : 2026-05-06
**Phase** : 18 — Matanga × FrieslandCampina × Brand Tree
**Supersedes** : aucun
**Related** : [ADR-0048](0048-glory-tools-as-primary-api-surface.md) (Glory tools as primary API surface), [ADR-0049](0049-brief-mandatory-gate.md) (brief mandatory gate), [ADR-0052](0052-brand-tree-multi-archetype.md) (Brand Tree dépend de cet invariant), [ADR-0055](0055-morning-brief-batch-validation.md) (Morning Brief Batch est l'application directe), [NEFER.md §1.1](../NEFER.md) (doctrine LLM)
...
**Phase 18-A0** (Brand Tree) — chaque Intent kind `OPERATOR_*_BRAND_NODE` ([ADR-0052 §4](0052-brand-tree-multi-archetype.md)) est exposé en route tRPC `brandNode.create/update/move/delete/attachStrategy/tagRole`. UI manuelle `<BrandNodeForm />` accessible depuis :
...
**Phase 18-A1** (Morning Brief Batch — cf. [ADR-0055](0055-morning-brief-batch-validation.md)) — chaque `BriefIngestionDraft` traverse le middle portal `/console/operate/morning-intake` avant matérialisation. Fonctions accessibles manuellement :
...
- **Confidence threshold configurable** (cf. [ADR-0055 §11](0055-morning-brief-batch-validation.md)) : `BriefIngestionDraft.confidence > 0.92` peut auto-confirmer
...
- [ ] `CHANGELOG.md` v6.18.15 entry mentionne ADR-0053
