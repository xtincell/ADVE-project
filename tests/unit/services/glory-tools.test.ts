...
describe("GLORY Tools — Registre", () => {
  it("doit contenir exactement 47 outils (40 legacy + 7 Phase 13 Oracle)", () => {
    expect(ALL_GLORY_TOOLS).toHaveLength(47);
  });
...
describe("GLORY Tools — 4 Layers", () => {
  ...
  it("doit avoir 16 outils dans la couche DC (Direction de Creation, +7 Phase 13)", () => {
    // 9 legacy DC + 7 Phase 13 Oracle (mckinsey-7s, bcg-portfolio, mckinsey-3-horizons,
    // overton-window, cult-index-scorer, bain-nps-calculator, tarsis-signal-detector)
    expect(getToolsByLayer("DC")).toHaveLength(16);
  });
  ...
  it("la somme des layers doit egaliser 47 (40 legacy + 7 Phase 13)", () => {
    const total =
