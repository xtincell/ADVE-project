...
## ~~Intake `processIngest` synchrone → « Load failed »~~ — **CLOS 2026-07-19** (F1 brief scoreur/intake, session dédiée)

**Root fix shippé** (v6.27.223) : `processShort`/`processIngest`/`processIngestPlus` rendent la main immédiatement (`{ status: "PROCESSING" }`) après persistance des sources + pré-flight LLM synchrone honnête + **claim atomique** (`IN_PROGRESS|FAILED → PROCESSING`) ; le travail lourd tourne en fire-and-forget (pattern scoreur `footprint.ts` — serveur long-vivant Coolify, pas de file externe) avec **transition terminale garantie** (`COMPLETED` écrit par `complete()`, sinon `FAILED` + `failureReason` ; garde paresseuse `getByToken` > 10 min → `FAILED "timeout"` ; `expireStale` couvre `FAILED` > 7 j). Statuts additifs `PROCESSING`/`FAILED` + `failureReason` (migration `20260719190000`, backfill-safe). Clients : hook `use-intake-processing-watch` (sondage 4 s jusqu'au statut terminal RÉEL) sur les 3 pages + `/result` ; retry sans perte (`advance()` rouvre une row `FAILED`). Vérifié E2E local `npm run verify:intake-async` (13 assertions, vraies procédures via caller). Le levier infra Traefik n'est plus nécessaire. Lignes PATCHED-SYMPTOMS convergentes purgées le même jour.

## Accents hors funnel — surfaces cockpit/console restantes (2026-07-19, NEFER)

F5 (brief scoreur/intake, v6.27.223) a restauré les accents sur TOUT le funnel : banque de questions serveur (`question-bank.ts` + banques sœurs `index.ts` diagnostic / `narrative-report.ts` / `brand-level-evaluator.ts`), labels `business-context.ts`, dict fr `intake-result.ts` (~98 chaînes + nom de marque « La Fusée » ×16 fr/en/zh), page publique `/score`. **Restent des chaînes FR sans accents hors funnel** (repérées au grep, non traitées — hors mandat) : `components/cockpit/notoria/notoria-page.tsx` (« Authenticite »), `components/cockpit/pillar-page.tsx` (« Strategie »), `components/shared/pipeline-progress.tsx`, `app/(console)/console/mestor/recos` + `console/artemis/skill-tree`, `app/(agency)/agency/intake` (« Completes ») — et les dicts i18n des pages non-intake à inventorier. **Plan** : sweep dédié par surface, même méthode déterministe que F5 (remplacements exacts assertés, jamais de regex aveugle), cockpit d'abord (client-facing), console/agency ensuite ; option structurelle = étendre le test HARD `cockpit-vocabulary` d'une garde anti-sans-accents sur les mots fréquents (`Authenticite|Strategie|communaute|coherence…`). **Déclencheur de reprise** : prochaine session UI cockpit OU prochaine extension du test `cockpit-vocabulary`. **Effort** : faible-moyen (mécanique, ~1 h).
...
- ~~**Ingestion externe générique push (PR-B)**~~ — **shippé ([ADR-0146](adr/0146-external-metric-ingestion-agnostic.md), v6.27.160)** : `INGEST_EXTERNAL_METRIC` (governor ANUBIS, zéro LLM) + `POST /api/ingest/metrics` (token MCP scopé ADR-0145 : BRAND ne pousse que sa marque, ou CRON_SECRET interne) → route par cellule vers `CampaignAARRMetric` (idempotent), `MissionActivity.kpiActual` (garde de portée) et un `Signal type=EXTERNAL_METRIC` de provenance. La fiche mission affiche « Dernière remontée le … » (récence réelle 30 j), fini le « bientôt ». Agnostique (aucune forme de marque codée en dur), 0 migration. **Reste déféré** : (1) le **pull** (aller chercher chez le fournisseur — Brevo, un CRM) — le motif push est prêt, un cron `verifyCronSecret` peut pousser en interne, mais le connecteur pull par provider exige un choix opérateur (clés Brevo/HubSpot) ; (2) l'**exposition de l'ingestion comme outil MCP scopé** (au lieu de l'endpoint brut) — même chemin qu'ADR-0145 §4.
