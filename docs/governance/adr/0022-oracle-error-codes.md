# ADR-0022 — Oracle Error Codes : catalogue gouverné, sérialisation safe, triage admin
...
3. **Refondre la classe `Error` globale du repo** — out of scope ADR-0022. Les autres services (Ptah, Mestor, Seshat) garderont leurs erreurs natives ; seul le pipeline Oracle est gouverné par codes typés (parce que c'est le pipeline le plus exposé à l'opérateur — livrable phare).
