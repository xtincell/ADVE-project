...
### Amendement 2026-07-14 — Instagram Business Login (provider `instagram` dédié)

Blocage opérateur : « Facebook connecte mais pas Instagram — le problème est du côté de la Fusée » → « il faut un tout autre code pour Instagram : voici l'id de configuration `1548627253622815` ». **Diagnostic** : les deux plateformes passaient par le SEUL provider `meta` (Facebook Login), et Instagram n'était découvert que via l'edge `me/accounts → instagram_business_account` (compte pro RELIÉ à une Page FB connectée). Quand cet edge reste vide (compte non rattaché, permission non accordée via la Configuration Meta, `META_LOGIN_CONFIG_ID` qui *bypasse* les scopes bruts), aucune connexion IG ne se crée — d'où « FB oui, IG non ».

**Décision** : ajouter un provider PROPRE `instagram` implémentant **« Instagram API with Instagram Login »** (Business Login, GA 2024) — « un tout autre code », mot pour mot la demande. La connexion Instagram ne dépend PLUS d'une Page Facebook.

Différences structurelles (pourquoi un provider dédié, pas une option de `meta`) :

| Étape | `meta` (Facebook Login) | `instagram` (Instagram Business Login) |
|---|---|---|
| Autorisation | `facebook.com/…/dialog/oauth` (+ `config_id`) | `instagram.com/oauth/authorize` + `enable_fb_login=0` |
| Échange de code | `graph.facebook.com/…/oauth/access_token`, réponse OAuth plate | `api.instagram.com/oauth/access_token`, réponse **ENVELOPPÉE** `{data:[{access_token,user_id,permissions}]}` |
| Long-lived | `fb_exchange_token` (POST) | `ig_exchange_token` (GET `graph.instagram.com/access_token`) |
| Refresh | via `refresh_token` | **self-refresh** `ig_refresh_token` (présente le token lui-même, pas de refresh_token) |
| Découverte / API | `graph.facebook.com` (token de Page) | `graph.instagram.com` (token IG) |
| Scopes | `instagram_basic`, `instagram_content_publish`… | `instagram_business_basic`, `instagram_business_content_publish`, `instagram_business_manage_comments` |
| `client_id` | Meta App ID | **Instagram App ID** (aucun `config_id`) |

**On étend, on ne double pas** (interdit anti-doublon n°1) : le provider réutilise TOUT le socle `oauth-integrations` (state HMAC, chiffrement AES-GCM, cookie PKCE, `getPublicBaseUrl`), les routes `start`/`callback` génériques (branchées sur `config.id === "instagram"`), l'Intent gouverné `ANUBIS_SOCIAL_CONNECT_ACCOUNT` (union `provider` étendue à `instagram`), le modèle `SocialConnection`, la ventilation `FollowerSnapshot`, le hub « Mes réseaux ». Seuls le token exchange (enveloppe), le long-lived/refresh (verbes IG) et l'hôte de collecte/publication (`graph.instagram.com`) sont spécifiques — factorés en `exchangeInstagramCode` / `exchangeInstagramLongLivedToken` / `refreshInstagramLongLivedToken` + `parseInstagramShortTokenResponse` (fonction PURE, testée : déballage enveloppe + tolérance forme plate + `null` si pas d'access_token). `PROVIDER_FOR_PLATFORM.INSTAGRAM` bascule `meta → instagram` : le bouton « Connecter » de la ligne Instagram vise désormais le flow direct.

**Compatibilité héritée (Loi 1 — pas de régression silencieuse)** : les connexions IG déjà créées sous `meta` (via l'edge FB-Page) restent lisibles — leur `metadata.provider="meta"` route la collecte/publication vers `graph.facebook.com` ; les nouvelles (`provider="instagram"`) vers `graph.instagram.com`. `fetchFollowersForConnection` / `fetchPostsForConnection` / `publishToInstagram` choisissent l'hôte selon le provider stocké. La publication accepte `instagram_content_publish` (héritée) OU `instagram_business_content_publish` (nouvelle) — n'importe laquelle suffit.

**Config env** : `INSTAGRAM_OAUTH_CLIENT_ID` (App ID Instagram — défaut `1548627253622815`, valeur PUBLIQUE fournie par l'opérateur, surchargeable ; `""` retombe sur le défaut) + `INSTAGRAM_OAUTH_CLIENT_SECRET` (secret — EN ENV UNIQUEMENT, jamais commité ; sans lui le provider est indisponible, l'UI dit « bientôt disponible »).

**Restes (RESIDUAL-DEBT)** : l'inbox commentaires (`social-inbox`) et les insights privés (`social-insights`) restent codés pour `graph.facebook.com` — une connexion `instagram` y dégrade honnêtement (AUTH/OUTAGE), à rendre provider-aware dans la vague Inbox S3 ; App Review Meta pour ouvrir au public (comptes testeurs OK immédiatement).
...
- Instagram se connecte en DIRECT (provider `instagram`, ADR-0128 amendé 2026-07-14) — plus de dépendance à une Page Facebook ; les connexions IG héritées via `meta` restent opérées via `graph.facebook.com` (Loi 1).
