...
- **`/api/chat` — routage LLM Gateway (budget/coût/rate)** — 🟡 **modèle CORRIGÉ (purge dette)**, routage = chantier : le modèle est passé Sonnet 4 legacy → **Sonnet 5** (défaut canon Gateway). Reste : router le chat via un chemin STREAMING du Gateway pour cost-tracking/circuit-breaker/headroom — le Gateway n'expose aujourd'hui que `executeStructuredLLMCall` (sorties structurées), pas de surface streaming ; c'est un vrai chantier (surface streaming à bâtir). Sécurité déjà fermée (auth + `canAccessStrategy`). **Déclencheur** : passe LLM Gateway streaming.
