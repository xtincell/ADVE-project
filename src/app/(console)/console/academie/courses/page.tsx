"use client";

import { useState } from "react";
import { PageHeader } from "@/components/shared/page-header";
import { StatCard } from "@/components/shared/stat-card";
import { EmptyState } from "@/components/shared/empty-state";
import { Tabs } from "@/components/shared/tabs";
import {
  BookOpen,
  Users,
  Clock,
  CheckCircle,
  Plus,
  PlayCircle,
} from "lucide-react";

export default function CoursesPage() {
  const [activeTab, setActiveTab] = useState("all");

  const tabs = [
    { key: "all", label: "Tous", count: 0 },
    { key: "published", label: "Publies", count: 0 },
    { key: "draft", label: "Brouillons", count: 0 },
    { key: "archived", label: "Archives", count: 0 },
  ];

  return (
    <div className="space-y-6">
      <PageHeader
        title="Catalogue de cours"
        description="Gestion des formations et contenus pedagogiques"
        breadcrumbs={[
          { label: "Console", href: "/console" },
          { label: "Academie", href: "/console/academie" },
          { label: "Cours" },
        ]}
      >
        <button className="flex items-center gap-2 rounded-lg bg-white px-4 py-2 text-sm font-medium text-zinc-900 hover:bg-zinc-200">
          <Plus className="h-4 w-4" /> Nouveau cours
        </button>
      </PageHeader>

      {/* Stat Cards */}
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard title="Total cours" value={0} icon={BookOpen} />
        <StatCard title="Apprenants actifs" value={0} icon={Users} />
        <StatCard title="Duree totale" value="0h" icon={Clock} />
        <StatCard title="Taux completion" value="- %" icon={CheckCircle} />
      </div>

      {/* Tabs */}
      <Tabs tabs={tabs} activeTab={activeTab} onChange={setActiveTab} />

      {/* Empty state */}
      <EmptyState
        icon={PlayCircle}
        title="Aucun cours"
        description="Creez des cours pour former les creatifs et clients de l'ecosysteme."
      />
    </div>
  );
}
