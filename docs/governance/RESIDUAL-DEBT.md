...
- **MCP — resources[] + OAuth (2026-07-26, ADR-0182)** : (1) les **28 `resources[]`** déclarées dans les modules MCP (ex. `intelligence://pillars/{strategyId}`) ne sont câblées à AUCUN transport — le scoping BRAND des URIs de resource n'est pas trivial (pas de champ `strategyId` structuré comme sur les tools). Câblage = mapper chaque URI → une lecture scopée + exposer via `resources/list`+`resources/read` du serveur MCP. (2) **OAuth 2.1** (discovery RFC 9728 + DCR) pour que **claude.ai web/mobile** se connecte (v1 = header `x-api-key`, cible Claude Code/Desktop qui acceptent les headers). **Déclencheur** : (1) besoin d'exposer des ressources en lecture pure côté client MCP ; (2) demande de brancher claude.ai web hébergé (pas le CLI). Effort ~1 session (resources) / ~2 sessions (OAuth).
