...
- **Phase 17 — Deliverable Forge (output-first composition)** ([ADR-0037](docs/governance/adr/0037-output-first-deliverable-composition.md)) — 🚧 ADR figé, code à venir. Surface cockpit `/cockpit/operate/forge` qui inverse le flow : le founder pointe le `BrandAsset.kind` matériel cible → resolver remonte le DAG des briefs requis (via nouveau champ `GloryToolForgeOutput.requires?: BrandAssetKind[]`) → vault-matcher ré-utilise les briefs ACTIVE non-stale + génère les manquants → composer construit une `GlorySequence` runtime ad-hoc dispatchée via `sequence-executor` existant → grappe `BrandAsset` liée par `parentBrandAssetId`. **Cap APOGEE 7/7 préservé** (Artemis governor, sous-composant Propulsion, pas de nouveau Neter, pas de nouveau model Prisma). 1 nouveau Intent kind `COMPOSE_DELIVERABLE` (sync dispatcher) ré-émet `INVOKE_GLORY_TOOL` + `PTAH_MATERIALIZE_BRIEF` + `PROMOTE_BRAND_ASSET_TO_ACTIVE` existants. Découpage 6 commits atomiques documenté en notes implémentation ADR-0037.
