...
- **Digest crons — `KnowledgeEntry.sourceHash` sans contrainte unique (LOW-MED)** *(round-14a, vérif des fixes round-13c)* : le claim-then-send des crons digest (`founder-digest`, `anubis/digest-scheduler`) dédupe les re-fires SÉQUENTIELS (le dominant) via `findFirst(sourceHash)` — mais `sourceHash` (`String?`) n'a pas de contrainte unique, donc sous fire TRULY-concurrent (deux schedulers au même instant) le `findFirst→create` est un TOCTOU → deux `create` → deux emails. **Impact : doublon d'email rare, JAMAIS de corruption.** C'est le pattern d'idempotence ACCEPTÉ du codebase (même limite sur `webhooks/mobile-money` renewalHash). **Fermeture** : `@@unique([sourceHash])` sur `KnowledgeEntry` + traiter le `P2002` du `create` comme « déjà claim → skip » (+ try/catch per-user dans digest-scheduler pour ne pas aborter le run). **Prérequis** : audit de TOUS les writers `KnowledgeEntry.sourceHash` (mcp/intelligence, webhooks momo) + vérif d'absence de doublons existants en base AVANT la migration. **Effort** : ~½ session. **Déclencheur** : passe idempotence stricte / si un doublon de digest est observé en prod.
- **`createVersion` : instantané `PillarVersion` orphelin sur conflit de version (LOW, pré-existant)** *(round-14a)* : `createVersion` committe le `PillarVersion` sur le client `db` global À L'INTÉRIEUR de la tx du gateway ; si le persist conditionnel échoue (conflit de version → tx rollback), l'instantané déjà committé n'est PAS annulé → ligne d'historique orpheline. **Intégrité intacte** (`ROLLBACK_PILLAR` restaure par `versionId` explicite, pas par numéro) — pollution d'audit uniquement. Round-13a RÉDUIT l'occurrence (seulement sur conflit réel désormais). **Fermeture** : déplacer `createVersion` dans la tx du gateway (le gateway l'évite délibérément pour que createVersion trouve la ligne via `db` global — cf. commentaire l.324) OU sweep de nettoyage. **Effort** : ~½ session. **Déclencheur** : passe fiabilité versioning.
- **Publication sociale — double-POST sur panne DB TOTALE (LOW extrême)** *(round-16a, arête résiduelle après le fix)* : le fix round-16a résout le cas commun (POST réussi + persist échoué → le handler sort l'action de PUBLISHING vers EXECUTED best-effort → le cron ne restaure pas → pas de re-POST). Reste l'arête EXTRÊME où le persist ET le fallback `updateMany` échouent tous deux (DB totalement indisponible pendant deux écritures consécutives) → l'action reste PUBLISHING → le cron restaure SCHEDULED → re-POST au tick suivant. **Effet** : doublon de post réseau (irréversible), fenêtre extrêmement étroite. **Fermeture** : clé d'idempotence côté réseau (FB Graph n'en a pas) OU marqueur « posted » persisté AVANT le POST (nécessite un 2-phase). **Effort** : ~1 session. **Déclencheur** : si un doublon de post est observé en prod / passe fiabilité publication.
- **Dual-scheduler (daemon + GitHub Actions tous deux configurés) — doublons bénins (LOW)** *(round-15b active, round-16a vérifie)* : sur la topologie belt-and-suspenders (daemon ON ET `scheduled-ops.yml` avec `PROD_BASE_URL`/`CRON_SECRET`), chaque route tire 2× par fenêtre (le curl GitHub court-circuite le `claimOnce` daemon). Vérifié SANS corruption : `social-sync` (claim atomique), `jehuty-refresh` (`claimOnce` route + overwrite déterministe), `argos-hunt` (upsert `ref` déterministe → 1 seul article, seul coût LLM ×2), `community-snapshot` (fenêtre vélocité `[J-45,J-7]` exclut les doublons même-jour → pas d'inflation cult). Reste : `anubis-digest`/`lead-nurture` = 1 email dupliqué possible sous fire TRULY-concurrent (même TOCTOU `sourceHash`/tag CRM que round-14a, même fermeture `@@unique`). **Fermeture** : `@@unique([sourceHash])` (cf. entrée round-14a) + choisir UNE topologie (daemon OU GitHub, pas les deux) en prod. **Effort** : ~½ session. **Déclencheur** : passe idempotence stricte / si doublons observés.
