...
- **Détection notoria des CELLULES de matrice partielles (LOW)** : `findEmptyLeafPaths` s'arrête aux tableaux (un tableau est une feuille) → le moteur notoria voit les **objets imbriqués** vides (`prophecy.pioneers`) mais PAS les **cellules vides d'un item de matrice existant** (`produitsCatalogue[2].gainClientConcret` sur un tableau non vide). **Honnête aujourd'hui** : ces cellules SONT remplies par l'auto-filler « Enrichir » (Phase 3, `expandArrayItemRequirements` sur `array_items_complete`) — donc couvertes par UNE des deux voies. Le bug rapporté (pilier A) portait sur des objets imbriqués, couverts. **Fermeture** : dans `engine.ts`, après `findEmptyLeafPaths`, éclater les items de matrice existants en cellules manquantes (réutiliser le patron `expandArrayItemRequirements`) et les lister au prompt (SET `arr[i].leaf` — le gateway les applique désormais, cf. ADR-0177). **Déclencheur** : si l'opérateur constate que la notoria (hub) laisse des cellules de matrice vides sur pilier V/D. Effort ~½ session.
